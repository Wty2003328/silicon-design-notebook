...
### 6.4 The `cpuidle` contract and the prediction problem
...
2. **Latency risk.** Each down-transition is a bet that demand will stay low. Losing the bet costs 50 µs of up-transition latency added to whatever operation triggered the demand, and that latency is user-visible in a way the energy is not (the §6.4 asymmetry again).
...
5. Arm Limited, *AMBA Low Power Interface Specification: Q-Channel and P-Channel*, document IHI 0068. The normative handshake this page derives from first principles in §4.
6. Arm Limited, *System Control and Management Interface (SCMI)*, document DEN 0056. The message protocol by which OS, hypervisor, and secure-world agents submit power, performance, clock, and sensor requests to a system control processor (§2.5, §12.3).
7. IEEE Std 1801, *Standard for Design and Verification of Low-Power, Energy-Aware Electronic Systems* (Unified Power Format). The power state table and the power-intent semantics referenced throughout §3.4 and §14.1.
8. D. Ernst, N. S. Kim, S. Das, et al., "Razor: A Low-Power Pipeline Based on Circuit-Level Timing Speculation," *Proceedings of MICRO-36*, 2003. The in-situ error-detection technique and the "better than worst case" argument of §9.2.
9. C. R. Lefurgy, A. J. Drake, M. S. Floyd, et al., "Active Management of Timing Guardband to Save Energy in POWER7," *Proceedings of MICRO-44*, 2011. Critical-path monitors driving a runtime voltage and frequency loop on a shipping processor; the quantitative basis for §9.3's recovery figures.
10. K. A. Bowman, J. W. Tschanz, N. S. Kim, et al., "A 45 nm Resilient Microprocessor Core for Dynamic Variation Tolerance," *IEEE Journal of Solid-State Circuits*, vol. 46, no. 1, 2011. Error-detection sequential circuits with clock stretching; the mechanism of §10.5.
11. R. Jejurikar, C. Pereira, and R. Gupta, "Leakage Aware Dynamic Voltage Scaling for Real-Time Embedded Systems," *Proceedings of DAC*, 2004. The critical-speed result that §8.3 rederives in the form used by modern platform characterization.
12. M. Weiser, B. Welch, A. Demers, and S. Shenker, "Scheduling for Reduced CPU Energy," *Proceedings of OSDI*, 1994. The original crawl-to-deadline formulation, useful as the contrast case in §8.4.
13. Linux kernel documentation, `Documentation/admin-guide/pm/cpuidle.rst`, `cpufreq.rst`, and `intel_pstate.rst`. The `menu`/`teo` governors, the `schedutil` control law and its headroom factor, PM QoS, and frequency-invariant utilization (§6.4, §7.2, §7.7).
14. Intel Corporation, *Intel 64 and IA-32 Architectures Software Developer's Manual*, Volume 3B, chapters on power and thermal management. RAPL domains and semantics, `APERF`/`MPERF`, `IA32_THERM_STATUS`, performance-limit-reason reporting, and the PL1/PL2/Tau control law (§11.2, §13.1).
15. M. Lipp, A. Kogler, D. Oswald, et al., "PLATYPUS: Software-based Power Side-Channel Attacks on x86," *IEEE Symposium on Security and Privacy*, 2021. Why fine-grained power telemetry is now access-restricted and quantized (§13.4).
