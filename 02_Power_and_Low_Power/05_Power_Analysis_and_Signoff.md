...
> **Prerequisites:** [Power_Fundamentals](01_Power_Fundamentals.md) (the total-power equation $P=\alpha C V^2 f + I_{leak}V$ and the three ceilings — thermal, delivery, energy — that signoff checks against), [Block_Activity_and_Power](02_Block_Activity_and_Power.md) (how the activity factor $\alpha$ is estimated, vectored vs vectorless, glitch power — this page *consumes* those estimates and does not re-derive them), [CMOS_Fundamentals](../00_Fundamentals/01_CMOS_Fundamentals.md) §4 (the $\tfrac12CV^2$ per transition and the delay-vs-$V_{DD}$ relation §9 leans on).
