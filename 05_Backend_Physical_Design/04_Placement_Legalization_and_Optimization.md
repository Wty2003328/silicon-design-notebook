...
Global utilization (cell area / placeable area) of **60–75%** is the normal band at advanced nodes — lower for control-heavy high-fanout logic, higher for regular datapaths. What actually predicts routability is **local** density in a moving window (20 µm × 20 µm is common), capped at **80–85%**, and the relationship is nonlinear near the knee: past ~75% global utilization each additional 5% typically raises total overflow 15–30%, because the whitespace that absorbed detours has run out.
...
Take a 1 mm × 1 mm block with 20,000 roughly uniform scan flops. For a **random** order the expected Manhattan distance between two independent uniform points in a unit square is $2/3$ per axis, so $\approx0.667$ mm per link and $\text{WL}\approx13.3$ m. For a **spatially ordered** chain the tour is essentially a travelling-salesman path of length $\beta\sqrt{nA}$ with $\beta\approx0.71$ (the Beardwood–Halton–Hammersley constant), giving $0.71\sqrt{20{,}000\times1\ \text{mm}^2}\approx100$ mm — **a factor of ~130**, more leverage than any other single placement transform.
...
The contract: with `scan_en` high, each `Q` must reach the next flop's `SI` within one shift cycle. The race is **not** against the period — at 20 MHz there is 50 ns of setup slack — it is against the **same edge**, since `FF_i+1` must not capture `b1` on the very edge that launched it from `FF_i`. Before reordering the 667 µm wire contributed ~130 ps and the race was safe by accident; after, the only defense is $t_{cq} > t_{\text{hold}}+t_{\text{skew}}$, which at 60 ps of shift skew it is not.
...
| Global utilization / local density cap | 60–75% global; 80–85% per 20 µm window | the numbers that predict routability (§7.2) |
...
| Shift-mode hold fix after reorder | $\approx40$ ps ≈ 2 buffers per link | the cost of the reorder trade (§8.3) |
| Pre-CTS / post-CTS clock uncertainty | 80–150 ps / 40–70 ps | the stand-in for a clock tree that does not exist yet (§11) |
