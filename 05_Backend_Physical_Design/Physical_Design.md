...
- **DRC / LVS / ERC / antenna / density-fill** — what each check is and how signoff runs it: [Physical_Verification_DRC_LVS](../06_Signoff/Physical_Verification_DRC_LVS.md) (the in-practice detail formerly in this section lives there).
- **Electromigration** — Black's equation, per-layer current limits, self-heating: [Signal_Integrity_Reliability](Signal_Integrity_Reliability.md) §4.
- **IR drop (static + dynamic)** — grid resistance models, vectorless vs vector-based, fixing: [Signal_Integrity_Reliability](Signal_Integrity_Reliability.md) §5 (grid design in §6).
- Power-integrity signoff criteria and tools (Voltus/RedHawk): [Power_Analysis_and_Signoff](../02_Power_and_Low_Power/Power_Analysis_and_Signoff.md).
