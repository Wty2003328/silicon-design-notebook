...
flowchart TD
    S0["Stage 0 · Reconditioning<br/>a gate is a circuit, a flop is a race<br/>7-10 weeks"]
    S1["Stage 1 · Frontend engineer<br/>synthesizable RTL you can defend,<br/>verified on purpose<br/>11-16 weeks"]
    S2["Stage 2 · Implementation engineer<br/>constraints, gates, geometry,<br/>timing, test, signoff<br/>20-28 weeks"]
    S3["Stage 3 · Architect<br/>where the numbers come from<br/>before anyone writes RTL<br/>20-28 weeks"]
...
**Total elapsed time to the end of Stage 3, at 10–15 focused hours per week: roughly 13 to 19 months.** That is the honest number, and it is arithmetic rather than a guess: the hour column in every table below is derived from the target page's actual length at a stated rate, and §10 shows the sum. It went *up* at the last recalibration for one reason: the power track (folder 02) roughly tripled in length and gained two new pages — the power-gating circuit and sequence, and the runtime controller — so the rows that point at it now cost what they actually cost, and the two new pages had to be placed. It is faster with a job that forces daily practice, and slower without hands-on tool access. Nothing about this is a race — a person who spends two years and can derive every result is in a far better position than one who spends four months and recognizes vocabulary.

**What "read this" means, now that there are 156 flow pages.** A path that implies you read all of them is a lie. The notebook is about 1.4 million words; at the rate in §10 that is 1,400 to 2,000 hours, and no working engineer has read their own reference cover to cover. So every reading list below is in two parts. The numbered rows are the **required core** — the material the next stage is genuinely not derivable without, and the only material the hour totals count. Each stage from 1 onward then ends with a **return when you need it** table: whole pages, or named sections of pages, that are fully part of the field and deliberately not part of the on-ramp. They carry their hours so you can see what they cost the day your work asks for them. Across the three shelves there are twenty distinct entries totalling 410–576 hours — more than any single stage — which is simply the shape of a reference as opposed to a curriculum.
...
| 7 | [Power Fundamentals](02_Power_and_Low_Power/01_Power_Fundamentals.md) | $P = \alpha C V^2 f$ derived, not quoted, with the short-circuit term derived rather than asserted and a clock-tree budget computed rather than quoted; the four leakage mechanisms including GIDL, the stack effect, subthreshold swing, and where "2× per 10 °C" comes from; $V_{DD}$ as the master knob and why DVFS is near-cubic; energy per op, the minimum-energy point, and EDP/$ED^2$ as the metric that arbitrates when power alone cannot; why Dennard scaling ended and what that did to the field. It tripled in length at the last revision, and the hours went with it — that is the right size for it, because most of the power arithmetic in the rest of the notebook is derived here and used everywhere else. | 17–23 |
...
**Cross-cutting, read alongside:** [Block Activity and Power](02_Power_and_Low_Power/02_Block_Activity_and_Power.md) §§1–2 and §5 — what the activity factor is, the two ways to get one, and what a glitch actually costs — and [Power Reduction Techniques](02_Power_and_Low_Power/04_Power_Reduction_Techniques.md) §§1–2, the lever map and the gating section, so that clock gating and operand isolation enter your hands as RTL habits rather than as someone else's later problem. Both pages are much longer than those sections; the rest of them is budgeting and implementation work that Stages 2 and 3 come back for. Also [Design Methodology and EDA Infrastructure](08_Cross_Cutting_Engineering/03_Design_Methodology_and_EDA_Infrastructure.md) §1 — who owes what to whom on a real project, and the milestone ladder your RTL will be judged against — and its §10 on design reviews, whose §10.3 is a concrete RTL review checklist. Read that checklist *before* project 1 and run it against your own code every time. Reviewing RTL is a separate skill from writing it, it is a large fraction of what a senior engineer's week actually contains, and Stage 2 asks you to do it to someone else's design.
...
**This stage is long enough to need a midpoint.** At 20–28 weeks it is, together with Stage 3, one of the two largest blocks in the path, and reading it as one undifferentiated list is how people stall in it. It divides cleanly in two, and the division is not cosmetic — it is the netlist. **Block 2A (rows 1–6) takes RTL to a gate netlist you can defend**, and ends at a checkpoint you can state: you can write the constraints, read the synthesis reports, and say what the design costs in gates and where its critical path is. **Block 2B (rows 7–24) takes that netlist to geometry, power intent, and a signoff package.** If you need to pause, pause between them — 2A is a coherent competence on its own, and it is the half a verification engineer or an architect can stop at without being crippled. 2B is where you become someone who ships.
...
| 20 | [Power Analysis and Signoff](02_Power_and_Low_Power/06_Power_Analysis_and_Signoff.md) | How power is finally proved rather than estimated: the accuracy ladder, vectorless versus annotated analysis, the peak-versus-average distinction, and top-down budget allocation against bottom-up closure — then the whole of power *integrity*, which is the half of this page a Stage 2 engineer meets first: peak power and regulator sizing, the supply network as one impedance problem, static IR drop as a resistive floor and dynamic IR drop as the transient that actually fails silicon, decap tiers and their time constants, electromigration and the Blech length, voltage-aware STA, the corner and margin trade, thermal with an RC model and $di/dt$, and the two test-mode power numbers of which one costs yield. This grew from a survey into the notebook's power-integrity page; if row 12 left you able to name IR drop and EM but not compute either, this is where that is fixed. | 19–26 |
| 21 | [Power Gating, Retention, and Wake Sequencing](02_Power_and_Low_Power/07_Power_Gating_Retention_and_Wake_Sequencing.md) — §§1–2, §§8–12 | The hardware that row 19's UPF file actually instantiates. What removing a supply does to a block; the header/footer switch and the virtual rail it creates; retention-flop topologies and what a checkpoint policy should really keep; the always-on domain and always-on routing; isolation cells with a *derived* clamp value rather than a defaulted one; level shifters at a gated boundary; and §12, the complete power-down and power-up sequences edge by edge, with the failure each ordering constraint prevents. Row 18 is the handshake, row 19 is the contract, and this row is the circuit and the order of operations — exit-check question 8 wants all three. The sizing, placement, and productionizing half of the page is on the shelf. | 11–16 |
| 22 | [Fabrication Process](07_Manufacturing_and_Bringup/01_Fabrication_Process.md) | Lithography and why it is the problem the field orbits; multi-patterning vs EUV; variation as a distribution; yield, $D_0$, and the economics that decide die size. | 6–8 |
| 23 | [IC Packaging](07_Manufacturing_and_Bringup/02_IC_Packaging.md) | The four jobs of a package; wire-bond vs flip-chip; power delivery into a hundred-amp load; the thermal stack; the reticle-and-yield inflection that produced chiplets; 2.5D and 3D. | 5–7 |
| 24 | [Tape-out and Post-Silicon Bring-up](07_Manufacturing_and_Bringup/03_Tapeout_and_Post_Silicon_Bringup.md) | What happens when the chip comes back: §4's bring-up ladder, §5 on debugging under an observability collapse, and §6's shmoo as a diagnostic — this is the notebook's account of **debugging silicon**, and it is the last constraint your course let you ignore, namely that the thing you must debug is now a physical object you cannot single-step. Short, and a map of post-silicon rather than a manual for it. | 3–4 |
...
| [Power Gating, Retention, and Wake Sequencing](02_Power_and_Low_Power/07_Power_Gating_Retention_and_Wake_Sequencing.md) — §§3–7, §§13–14 | The half of the page that turns "we will gate this block" into a buildable fabric: sizing the switch fabric from a droop budget, fine- versus coarse-grain, where the switches physically go, rush current and the two-stage daisy-chained turn-on that keeps a wake from browning out the neighbours, break-even residency in both its energy and its latency form, what gating does to characterization, STA, CTS, IR/EM, and DFT, and a catalogue of the failures that reach silicon. Return when a power-gated block is yours rather than something you are reading about. Row 21 is the part the on-ramp needs. | 11–16 |
...
| 18 | [Low-Power Architecture and Domain Partitioning](02_Power_and_Low_Power/03_Low_Power_Architecture_and_Domain_Partitioning.md) | Where domain boundaries come from, and why partitioning is an architectural decision that Stage 2's UPF row merely records. Power, voltage, and clock domains as three different questions; the inputs that must exist before a boundary can be drawn; the power- and voltage-domain partition cost models *evaluated numerically* rather than described; the regulator section — LDO, buck, switched-capacitor, and integrated regulator — which is where "give it its own voltage" acquires a price; the domain signature that co-partitions the axes; a repeatable workflow and a worked SoC partition; power-aware DFT as a partition constraint; and the handoff checklist. It quadrupled in length at the last revision and is now the architect's power page. | 25–34 |
| 19 | [Runtime Power Management and AVFS](02_Power_and_Low_Power/08_Runtime_Power_Management_and_Adaptive_Voltage_Frequency.md) — §§1–4, §§6–8 | The controller that decides when to use everything row 18 partitioned. The baseline policy and the five ways it fails; the power-management unit as hardware, interface by interface; power state machines and the power state table the FSM must stay inside; the quiescence handshake derived from first principles and then mapped onto AMBA Q-Channel and P-Channel, so Stage 2 row 18's four signals acquire the reason they have those semantics; the software interface — ACPI C- and P-states, Arm PSCI, Linux `cpuidle` — which is the boundary an architect specifies and a driver writer lives on; DVFS as a control loop with governors rather than as a knob; and §8's race-to-idle versus crawl-to-deadline, worked to an interior optimum that is neither. It also supplies the idle-state half of exit-check question 7. The circuit-level and thermal material is on the shelf. | 16–22 |
...
| [Runtime Power Management and AVFS](02_Power_and_Low_Power/08_Runtime_Power_Management_and_Adaptive_Voltage_Frequency.md) — §5, §§9–14 | **Required for anyone who owns a power controller rather than specifies one.** What clock and reset do *inside* a transition; AVS and AVFS as guardband recovery, in millivolts and then in watts; droop and adaptive clocking; thermal management and why a part inside its power budget still throttles; granularity and the multi-agent problem, which is what happens when several loops pull the same rail; telemetry; and §14's five formal properties that catch the bug classes simulation will not. Return when the controller is a thing you are building, debugging, or verifying. Row 19 is the part an architect needs. | 11–16 |
...
| Stage 0 | 80–111 h reading + 8–12 h building → **7–10 weeks** at 12 h/week | Removes the free-gate, free-wire assumption |
| Stage 1 | 99–138 h reading + 35–50 h building → **11–16 weeks** | The two skills, design and verification, interleaved |
| Stage 2 | 203–284 h reading + 35–50 h building → **20–28 weeks** | With Stage 3, one of the two largest blocks; also the highest-leverage. Split at the netlist into blocks 2A and 2B so that it has a usable midpoint |
| Stage 3 | 198–276 h reading + 40–60 h building → **20–28 weeks** for one architecture book | Depth in one beats breadth in four. Costed on the CPU book; the other three land within a few weeks |
| The three shelves | 410–576 h across twenty entries, **counted in none of the rows above** | Depth on demand. Reading it early is the failure mode of §11, not diligence |
| Elapsed to end of Stage 3 | 58–82 weeks ≈ **13–19 months** at 10–15 h/week | The honest number, and it is the sum of the four stage rows above |
