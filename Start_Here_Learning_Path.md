...
**Why this stage exists.** This is where most self-taught engineers stop, and it is exactly the material that separates someone who writes RTL from someone who ships silicon. It is also where the notebook's biggest recent additions live, because it is the part of the flow that is hardest to learn without a job. Read it even if you intend to be a verification engineer or an architect: a verification engineer who does not know what gate-level simulation is checking, or an architect who does not know why their proposed 8-read-port register file is physically absurd, is limited in a way that shows.
...
5. **Compute a power grid** for your block from its estimated current and check it against the IR-drop budget by hand, then compare to what the tool says.
...
| # | Page | Why |
|---|---|---|
| 1 | [Architecture and PPA — section index](01_Architecture_and_PPA/00_Index.md) | The framing and the four-book structure. |
| 2 | The design-methodology sub-book of your chosen architecture — [CPU](01_Architecture_and_PPA/01_CPU_Architecture/00_Design_Methodology/00_Index.md), [GPU](01_Architecture_and_PPA/02_GPU_Architecture/00_Design_Methodology/00_Index.md), [NPU](01_Architecture_and_PPA/03_NPU_Architecture/00_Design_Methodology/00_Index.md), or [SoC/chiplet](01_Architecture_and_PPA/04_SoC_and_Chiplet_Architecture/00_Design_Methodology/00_Index.md) | Workload definition, design-space exploration, PPA estimation, and what counts as evidence. Every one of these introduces its terms before using them. |
| 3 | [SystemC and TLM](00_Fundamentals/05_SystemC_and_TLM.md) | The modeling substrate: discrete-event simulation, delta cycles, transaction-level modeling. You cannot evaluate an architecture you cannot model. |
| 4 | [Research-Depth and Evidence Standard](Research_Depth_and_Evidence_Standard.md) | The rubric for what a defensible architectural claim looks like. Read it before you make one. |

**The CPU path**, as the worked example — the other books have parallel structures:

[Core foundations](01_Architecture_and_PPA/01_CPU_Architecture/01_Core_Foundations/00_Index.md) → [RISC-V ISA](01_Architecture_and_PPA/01_CPU_Architecture/01_Core_Foundations/02_RISC_V_ISA.md) → [frontend and prediction](01_Architecture_and_PPA/01_CPU_Architecture/02_Frontend_and_Prediction/00_Index.md) → [out-of-order backend](01_Architecture_and_PPA/01_CPU_Architecture/03_Out_of_Order_Backend/00_Index.md) → [cache hierarchy](01_Architecture_and_PPA/01_CPU_Architecture/04_Cache_Hierarchy/00_Index.md) → [virtual memory](01_Architecture_and_PPA/01_CPU_Architecture/05_Virtual_Memory/00_Index.md) → [coherence and consistency](01_Architecture_and_PPA/01_CPU_Architecture/06_Coherence_and_Consistency/00_Index.md) → [case studies](01_Architecture_and_PPA/01_CPU_Architecture/07_Core_Case_Studies/00_Index.md) → [simulation](01_Architecture_and_PPA/01_CPU_Architecture/08_Simulation/00_Index.md) → [implementation blueprints](01_Architecture_and_PPA/01_CPU_Architecture/10_Implementation_Blueprints/00_Index.md).

**System-level material everyone needs**, whichever book you chose: [AHB/AXI/APB](01_Architecture_and_PPA/04_SoC_and_Chiplet_Architecture/03_Transaction_Protocols/01_AHB_AXI_APB.md), [DDR controller](01_Architecture_and_PPA/04_SoC_and_Chiplet_Architecture/02_Shared_Memory/01_DDR_Controller.md), [network-on-chip](01_Architecture_and_PPA/04_SoC_and_Chiplet_Architecture/04_On_Chip_Networks/01_Network_on_Chip.md), [high-speed I/O and peripherals](01_Architecture_and_PPA/04_SoC_and_Chiplet_Architecture/05_IO_and_Chiplets/03_High_Speed_IO_and_Peripheral_Protocols.md), and [low-power architecture and domain partitioning](02_Power_and_Low_Power/03_Low_Power_Architecture_and_Domain_Partitioning.md).

**Cross-cutting, now non-optional:** [IP Reuse, Integration, and Register Automation](08_Cross_Cutting_Engineering/04_IP_Reuse_Integration_and_Register_Automation.md) and [Design Methodology and EDA Infrastructure](08_Cross_Cutting_Engineering/03_Design_Methodology_and_EDA_Infrastructure.md). An architect who cannot reason about integration cost is proposing designs that cannot be built on schedule.
...
3. **Do a design-space exploration with a real budget:** pick a target, enumerate three microarchitectures, estimate area and power for each using the cost models from the PPA chapters, and write the one-page recommendation with its losing cases.
