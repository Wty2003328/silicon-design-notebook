...
### 4.2 Mixed-precision numerics and 2:4 structured sparsity

§4.1 warned that the matrix pipeline's peak "assumes the requested data type, instruction shape, and sparsity mode." Both are load-bearing, and in opposite ways: the input/accumulate **format pairing** sets the peak *and* whether the result is even correct, while the **sparsity mode** sets how many of the issued multiplies are real work. Derive each from the job it must do, as with the ROB fields (§4) and the reconvergence stack (§3).

**Why the multiplier is narrow but the accumulator must stay wide.** A matrix multiply–accumulate reduces along a contraction dimension $K$: every output is a dot product $\sum_{i=1}^{K} a_i b_i$ folded into $C$. Two different numeric pressures act on the two halves of that MAC.

- *The multiply* $a_i b_i$ is where area and energy live. A floating-point multiplier is an $O(p^2)$ partial-product array in the significand width $p$ ([Floating_Point §6](../../../00_Fundamentals/04_Floating_Point.md)), so halving the input mantissa roughly **quarters** the multiplier — the entire reason to feed a tensor core a narrow format: 16-bit floating point (FP16, $1\!\cdot\!5\!\cdot\!10$), brain floating-point 16-bit (BF16, $1\!\cdot\!8\!\cdot\!7$), TensorFloat-32 (TF32, $1\!\cdot\!8\!\cdot\!10$), 8-bit floating point (FP8), or 4-bit floating point (FP4). Narrow inputs are cheap.
- *The accumulate* is where **error** lives, and it cannot be narrowed. A $K$-term sum accumulates rounding error that grows with $K$ ([Floating_Point §6](../../../00_Fundamentals/04_Floating_Point.md)): worst case $\lesssim (K-1)\,u\sum_i|t_i|$, and as a zero-mean random walk $\sim\sqrt{K}\,u\,\|t\|$, where $u=2^{-p_{acc}}$ is the *accumulator's* unit roundoff.

Derive the required accumulator width. To retain $b$ good bits in the result the accumulated relative error must stay below $2^{-b}$, so (worst case) $K\,2^{-p_{acc}}\lesssim 2^{-b}$, i.e.

$$p_{acc}\;\gtrsim\; b+\log_2 K\quad(\text{worst case}),\qquad p_{acc}\;\gtrsim\; b+\tfrac12\log_2 K\quad(\text{stochastic}).$$

The accumulator must carry **about $\log_2 K$ mantissa bits more than the target precision.** The failure it averts is *swamping*: once the running sum exceeds a term by more than the accumulator's unit-in-the-last-place, that term rounds entirely away, so accumulating a long reduction in the *input's* narrow precision silently discards most of the products. Hence the universal tensor-core datapath — and the reason §4.1's accumulator fragments "remain live across the reduction dimension": **multiply narrow, accumulate wide**, FP16/BF16/FP8 in, a 32-bit floating point (FP32) accumulator out. *Worked number:* a $K=4096$ reduction needs $\log_2 4096 = 12$ extra mantissa bits (worst case), 6 (stochastic). Accumulate it in FP8 E4M3 (significand $p_{acc}=4$, unit roundoff $u=2^{-4}$) and the error is $\sim 4096\times2^{-4}=256\times$ the terms — pure noise; accumulate in FP32 ($p_{acc}=24$, $u=2^{-24}$) and it is $4096\times2^{-24}=2^{-12}$ relative, twelve good bits, adequate. The narrow multiplier was free; a narrow accumulator would have destroyed the GEMM (general matrix multiplication).

**The format ladder (peak).** The $1/p^2$ *multiplier-area* law is an upper bound on what narrowing buys — on its own a halved mantissa would quadruple density — but shipped tensor cores step in **2×** increments, because the FP32 accumulator, operand delivery, and register-file bandwidth do not narrow with $p$ and come to dominate. Normalizing FP16/BF16 to $1\times$: TF32 $\approx\tfrac12\times$ (it carries FP16's 10-bit significand in a wider exponent field, so its half rate is a datapath/product decision, not the $1/p^2$ law), FP8 $\approx 2\times$, FP4 $\approx 4\times$. *Worked number:* an H100-class part sustains $\approx 990$ TFLOP/s dense FP16 and $\approx 1980$ TFLOP/s dense FP8; a Blackwell-class part adds FP4 for a further $\sim2\times$. A peak-TFLOP headline is therefore meaningless without naming the format, and the format is bounded *below* by the range/precision the workload tolerates ([Floating_Point §7](../../../00_Fundamentals/04_Floating_Point.md): training buys exponent range, inference spends its last bits on mantissa precision).

**2:4 structured sparsity (a second, orthogonal $2\times$).** Prune the weight matrix so that in every group of four consecutive weights along $K$ exactly two are nonzero. Store the two survivors densely plus, for each survivor, a **2-bit position index** naming its slot within the four — four metadata bits per group of four. The (sparse) tensor core reads that metadata and uses it to drive a $4\!\rightarrow\!2$ selection on the activations: of the four activations the group spans along $K$, only the two that pair with surviving weights are routed into the two MAC lanes. Half the products are never formed, so a given reduction finishes in half the MAC issues:

$$\text{peak}\;\propto\;\underbrace{\frac{1}{p_{in}^2}}_{\text{format}}\;\times\;\underbrace{\frac{1}{1-s}}_{\text{sparsity}},\qquad s=\tfrac12\ (\text{2:4})\;\Rightarrow\;2\times .$$
...
| Tensor-core format ladder | FP16/BF16 $1\times\to$ FP8 $2\times\to$ FP4 $4\times$ peak, FP32 accumulate | peak $\propto 1/p_{in}^2$; the TFLOP number is meaningless without the format (§4.2) |
| Accumulator width | $p_{acc}\gtrsim b+\log_2 K$ mantissa bits | why a long-$K$ dot product accumulates in FP32, not the input format (§4.2) |
| 2:4 structured sparsity | keep 2 of 4 + 2-bit index/survivor $\to$ up to $2\times$ | stacks on the format; needs retraining; the only sparsity a tensor core speeds up (§4.2) |
...
- **Numerics and sparsity (§4.2):** [Floating_Point](../../../00_Fundamentals/04_Floating_Point.md) (the range/precision trade, the $O(p^2)$ multiplier cost, and the multiply-narrow/accumulate-wide rule the format ladder rests on), [Sparsity, Quantization, and Compression](../../03_NPU_Architecture/02_Mapping_and_Memory/02_Sparsity_Quantization_and_Compression.md) (the 2:4 select datapath, unstructured sparsity, and metadata alignment).
...
[Section Index](00_Index.md) · [Root Index](../../../Index.md) · next ➡ [Single Instruction, Multiple Threads (SIMT) Scheduling and Occupancy](02_SIMT_Scheduling_and_Occupancy.md)
