...
The whole timing model reduces to a per-cycle decision the event engine ([Simulation_Methodology §3](../../05_Architecture_Foundations_and_Methods/05_Simulation_Methodology/01_Simulation_Methodology.md)) repeats for every scheduler on every SM. Each cycle the scheduler holds a set of resident warps $\mathcal W$; it computes the *ready* subset and picks one:
...
[Simulation_Methodology §4](../../05_Architecture_Foundations_and_Methods/05_Simulation_Methodology/01_Simulation_Methodology.md) writes the executed stream as a function of the microarchitecture, $\Pi(\mu)$: replaying a frozen trace $\Pi(\mu_0)$ into a timing model of $\mu_1$ is correct **iff $\Pi(\mu_1)=\Pi(\mu_0)$**, i.e. iff the path is $\mu$-independent. For an OoO CPU the dangerous term is *wrong-path*: the branch predictor is part of $\mu$, and a committed-only trace omits the $\phi_{\text{wp}}=b(1-a)W_{\text{wp}}$ speculative instructions ([Simulation_Methodology §4](../../05_Architecture_Foundations_and_Methods/05_Simulation_Methodology/01_Simulation_Methodology.md)) that pollute caches and contend for ports — a 30%-extra-fetch distortion on branchy, hard-to-predict code.
...
- **Up the stack:** [Simulation_Methodology](../../05_Architecture_Foundations_and_Methods/05_Simulation_Methodology/01_Simulation_Methodology.md) (the machinery this page instantiates for GPUs: the $(L,II,P)$ cost model §6 → §2.1, the event engine and emergent contention §3 → §3.1, execution-vs-trace path-dependence §4 → §5.1, workload→number §5 → §6, the error budget §8 → §8), [Performance_Modeling_and_DSE §9](../../05_Architecture_Foundations_and_Methods/02_Performance_Analysis/01_Performance_Modeling_and_DSE.md) (the GPU roofline/occupancy model this makes executable) and [§11 NeuSim](../../05_Architecture_Foundations_and_Methods/02_Performance_Analysis/01_Performance_Modeling_and_DSE.md) (the operator-level NPU analogue), [Full_Chip_Modeling §3](../../04_SoC_and_Chiplet_Architecture/01_System_Modeling/01_Full_Chip_Modeling.md) (GPU in the chip-level perf→power→thermal ladder), [Root Index](../../../Index.md).
