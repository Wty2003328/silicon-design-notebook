...
> **Prerequisites:** [CPU_Architecture](03_CPU_Architecture.md) §8–§9 (the coherence *contract* — SWMR, MESI, snoop-vs-directory — and the consistency model this page realizes), [AHB_AXI_APB](11_AHB_AXI_APB.md) (AXI4 channels and handshake, the substrate ACE extends), [Cache_Microarchitecture](07_Cache_Microarchitecture.md) §8 (private caches, MSHRs, inclusion — the caches being kept coherent).
> **Hands off to:** [Network_on_Chip](13_Network_on_Chip.md) (the mesh, routing, flow control, and deadlock the CHI protocol rides on), [DDR_Controller](10_DDR_Controller.md) (the memory subordinate at the far end), [GPU_Architecture](15_GPU_Architecture.md) & [NPU_Accelerators](16_NPU_Accelerators.md) (coherence *clients* over CXL).

---

## 0. Why this page exists

[CPU_Architecture §8](03_CPU_Architecture.md) handed you the coherence **contract** — the Single-Writer/Multiple-Reader invariant (for any line, at any instant, either one cache writes and no other holds, or any number read and none writes) and the Data-Value invariant (a read returns the last write in coherence order) — and the four MESI states each cache uses to play its part. But a protocol is not a state machine sitting inside one cache. **It is an agreement enforced across many caches, over a real fabric that has latency, races, and no global atomic bus.** The MESI diagram assumes a genie that can instantly tell a cache "you are the only copy" or "invalidate everyone else." This page is about building that genie out of wires.
...
Two design elements matter conceptually; the rest of ACE's transaction menu is just MESI transitions ([§8](03_CPU_Architecture.md)) named for the bus.
...
- **Down the stack (what this fabric is built from):** [Network_on_Chip](13_Network_on_Chip.md) (the mesh topology, credit-based flow control §3.3, routing/protocol deadlock and virtual networks §5, and the coherent-mesh/CMN realization §6 that CHI rides on; D2D physical layer §7), [AHB_AXI_APB](11_AHB_AXI_APB.md) (the AXI4 channels and handshakes ACE extends), [Memory](09_Memory.md) & [DDR_Controller](10_DDR_Controller.md) (the SN-F memory endpoint), [IC_Packaging](../07_Manufacturing_and_Bringup/02_IC_Packaging.md) (the chiplet/D2D substrate of §7).
- **Up the stack (what defines the contract this realizes):** [CPU_Architecture](03_CPU_Architecture.md) §8 (the SWMR/Data-Value invariants and MESI/MOESI states this page enforces on wires) and §9 (the consistency model the fabric's ordering honors), [Cache_Microarchitecture](07_Cache_Microarchitecture.md) §8 (the in-cache coherence controller, MSHRs, and inclusion policy on the other side of the snoop).
- **Adjacent / consumers:** [OoO_Execution](05_OoO_Execution.md) §5 (the LSQ that enforces per-core ordering the fabric completes), [GPU_Architecture](15_GPU_Architecture.md) & [NPU_Accelerators](16_NPU_Accelerators.md) (accelerators that become CXL coherence clients, §7), [Xiangshan_CPU_Design](14_Xiangshan_CPU_Design.md) (a complete core that plugs into such a fabric), [Performance_Modeling_and_DSE](01_Performance_Modeling_and_DSE.md) (where these scaling models feed system design-space exploration).
...
| Direction | Link |
|---|---|
| Prerequisite | [CPU_Architecture](03_CPU_Architecture.md) (coherence contract §8–§9) |
| Prerequisite | [AHB_AXI_APB](11_AHB_AXI_APB.md) (AXI4 substrate) |
| Hands off to | [Network_on_Chip](13_Network_on_Chip.md) (the fabric CHI rides on) |
