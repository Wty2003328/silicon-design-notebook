...
AMAT quietly assumes misses are *serial*. The escape is the same idea as pipelining — overlap. If the core can keep $N$ independent misses outstanding at once, their latencies overlap and the *effective* per-miss cost falls toward $t_{\text{miss}}/N$. This is **memory-level parallelism (MLP)**, and by Little's law the outstanding count the machine needs is the bandwidth–delay product $N = \lambda_{\text{miss}} \times t_{\text{miss}}$, where $\lambda_{\text{miss}}$ is the miss **rate per cycle** — misses per instruction times IPC — not misses per access. The units have to work: an occupancy is a rate times a latency, so a per-access miss ratio must first be converted into misses per unit time. (A loop at IPC 3 missing once every 50 instructions has $\lambda_{\text{miss}} = 3/50 = 0.06$ misses/cycle, so covering a 200-cycle miss needs $N = 12$ outstanding.) MLP is a distinct axis from instruction-level parallelism: a dependency-bound loop has low ILP but a strided access pattern can still expose high MLP.
