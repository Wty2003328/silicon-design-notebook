# Cache Microarchitecture — Locality, Average Memory Access Time (AMAT), and Controller Design

> **First-time reader orientation:** A cache keeps copies of recently used memory blocks because nearby or recent data is likely to be reused. A hit returns the copy; a miss fetches a cache line from a lower level. Average memory access time combines both outcomes, while controller state tracks outstanding misses, writes, replacements, and responses.

> **Abbreviation key — skim now and return as needed:** central processing unit (CPU); memory-level parallelism (MLP); design-space exploration (DSE); out-of-order (OoO); translation lookaside buffer (TLB);
> virtual page number (VPN); reorder buffer (ROB); miss status holding register (MSHR); load-store queue (LSQ); static random-access memory (SRAM);
> dynamic random-access memory (DRAM); high-bandwidth memory (HBM); double data rate (DDR); content-addressable memory (CAM); first in, first out (FIFO);
> level-one cache (L1); level-two cache (L2); level-three cache (L3); last-level cache (LLC); direct memory access (DMA);
> AXI Coherency Extensions (ACE); Coherent Hub Interface (CHI); Modified, Exclusive, Shared, Invalid (MESI); Modified, Owned, Exclusive, Shared, Invalid (MOESI); least recently used (LRU);
...
A cache exists to fight one number: the gap between a core cycle (~0.2–0.3 ns) and a DRAM access (~60–100 ns). That gap — the **memory wall** — is 100–300 core cycles wide, long enough to drain any realistic instruction window and leave the machine idle. The out-of-order core ([OoO_Execution](../03_Out_of_Order_Backend/01_OoO_Execution.md)) hides *some* of that latency behind a large window; the cache attacks the latency *itself*, and manages to do so cheaply only because real programs are not random.
...
```mermaid
flowchart LR
  IDX["Index"] --> TAG["Tag SRAM<br/>all N ways"]
  IDX --> DATA["Data SRAM<br/>all N ways"]
  CPU["CPU tag"] --> CMP["N comparators"]
  TAG --> CMP
  CMP -->|N match lines| HIT["OR reduce<br/>hit / miss"]
  DATA --> MUX["way-select MUX"] --> OUT["data out"]
  CMP -.->|winning way one-hot| MUX
```
...
### 2.3 Serving more than one access per cycle: the multiport design space

A superscalar core issues two or more loads per cycle ([Load-Store Unit](../03_Out_of_Order_Backend/02_Load_Store_Unit_and_Memory_Ordering.md)), so L1D must return several independent words per cycle. But an SRAM bit cell is natively **single-ported** — one wordline, one bitline pair, one access at a time. Getting $P$ accesses per cycle out of single-ported storage is one of the sharpest area/frequency trades in the core, and there are exactly four ways to buy it, each paying in a different currency.

**1. True multiport cells.** Give every bit cell $P$ physical ports — $P$ wordlines and $P$ bitline pairs threaded through each cell. Access is then unconditional: any $P$ addresses proceed with no conflict. The cost is quadratic — each added port runs a wire across *every* cell in both dimensions, so cell area grows roughly as $(\text{ports})^2$ (a 6T single-port cell becomes an 8T cell for two ports, $\sim$30–50 % larger, and it worsens from there), and the added capacitance on the shared nodes slows the access. This is affordable only for **small, port-hungry** structures — the register file carries 8–12 true ports ([OoO Execution](../03_Out_of_Order_Backend/01_OoO_Execution.md): the PRF is a heavily multiported RAM whose area and delay grow with port count) — never a 32–64 KB L1D, where 3–4 true ports would be prohibitive in both area and $t_{hit}$.

**2. Multi-banking (interleaving).** Split the array into $B$ independent, single-ported **banks** selected by low address bits; $P$ accesses proceed together *iff* they land in distinct banks, and same-bank requests arbitrate — one wins, the rest replay. That **bank conflict** is *not* a miss and must not train replacement or prefetch state. Area is roughly **linear** (more decoders, a $P\times B$ steering crossbar, arbitration), which is why banking is the **dominant L1D method**. Its throughput is *probabilistic*: two random accesses collide with probability $1/B$ (so $B=8$ banks leave a $\tfrac18$ conflict rate for a load pair), and a pathological same-bank stream serializes completely. More banks cut conflicts but grow the crossbar ($\sim P\cdot B$) and routing; XOR-hashing the bank-select bits spreads strided patterns that would otherwise camp on one bank — the same move the [HBM address map](../../02_GPU_Architecture/02_Memory_System/02_HBM_and_Advanced_Memory_Systems.md) (§4) makes one level down.

**3. Replication (mirroring).** Keep $C$ copies of the data array, each single-read-ported; $C$ readers each hit their own copy, giving $C$ **guaranteed, conflict-free read ports** at $C\times$ the area. The catch is writes: every store must update *all* $C$ copies, so replication buys read bandwidth but **not** write bandwidth, and it needs a write network to keep the copies identical. It pays off exactly when reads dominate writes (the usual case for L1D) — and because tags are far smaller than data, a common split is to **replicate the tag array** (cheap, gives every load pipe its own tag read) while **banking the data array**.

**4. Time-multiplexing (double-pumping).** Clock the single-ported SRAM at $2\times$ the core frequency (or access it on both clock edges), serving two core-level accesses through one physical port each core cycle. Area is that of a **plain single-port** array — the cheapest option — but it spends **frequency headroom**: it only works if the SRAM can cycle at $2\times$ core clock, which fails precisely when the cache is already the frequency-limiter, and it doubles the array's dynamic power. The Alpha 21264 double-pumped its 64 KB L1D to land two loads per cycle; modern high-frequency cores, whose L1 arrays already run near their speed limit, usually cannot — which is why banking has won.

| Technique | Area | Ports gained | Conflicts | Frequency / power | Typical use |
|---|---|---|---|---|---|
| True multiport | $\sim(\text{ports})^2$ | $P$, guaranteed | none | slower cell | register file, small CAMs |
| Multi-banking | linear + crossbar | up to $\min(P,B)$ | bank conflicts $\approx 1/B$ | neutral | **L1D data array** |
| Replication | $\times C$ | $C$ read, guaranteed | none (reads) | needs write network | tag arrays; read-heavy L1D |
| Double-pumping | single-port | $2\times$ | none | needs $2\times$ SRAM clock, $2\times$ power | Alpha 21264 L1D |

**The methodology is per-structure, not one-size.** The choice falls out of four measured quantities: the **read:write ratio** (read-heavy favours replication; write-heavy kills it), whether the consumer needs **guaranteed** or merely **probabilistic** throughput (an RF read for issue must never conflict → true ports; an L1D load can tolerate a rare replay → banking), the **area/frequency budget**, and the **offered-address distribution** (how often the simultaneous accesses actually collide in the same bank). So a register file takes true ports; an L1D data array is banked — often with *replicated tags*, a *store port kept separate from the load ports*, and, where the SRAM is fast enough, a touch of double-pumping. It is validated the way the [Implementation Blueprint](../10_Implementation_Blueprints/02_Memory_Translation_and_Coherence_Implementation_Blueprint.md) demands: choose the bank mapping so common simultaneous accesses distribute, then *prove* a bank conflict yields backpressure or replay — never silent loss — and count conflicts by requester class so the bank count is tuned against real traffic instead of a uniform-random guess.
...
*Worked number* — hide a $L_{miss}=100\text{ ns}$ DRAM miss while saturating one DDR-class channel of $B=25.6\text{ GB/s}$ with $G=64\text{ B}$ lines: the completion rate is $\lambda = 25.6\times10^{9}/64 = 4.0\times10^{8}$ misses/s, so $N \ge 4.0\times10^{8}\times100\times10^{-9} = \mathbf{40}$ outstanding misses. That is why an aggressive L1D/L2 path exposes tens of MSHRs: at 100 ns and 64 B lines you need ~40 in flight *just to keep one channel busy*, and proportionally more for multi-channel HBM-class bandwidth. Equivalently, at 3 GHz that 100 ns is ~300 cycles, so the demanded MLP to cut the effective penalty to ~10 cycles (§3.1) is $\approx 30$ — the same order. But $k$ is not free to grow: the *independent* misses available to overlap are limited by how far the instruction window can run ahead, which the ROB caps ([OoO_Execution §3.2](../03_Out_of_Order_Backend/01_OoO_Execution.md)). So MSHR count and window size are co-designed — an L1D that could track 40 misses is pointless behind a window that only ever exposes 8. This is why real MSHR counts track the level's realistic MLP, not the theoretical maximum:
...
*Scope: the idea and where it lands in a cache. The architectural stable-state contract lives in [CPU_Architecture §8](../01_Core_Foundations/01_CPU_Architecture.md); the full controller—transient states, message traces, races, directory formats, false sharing, atomics/DMA, safety/liveness, and verification—lives in [Cache_Coherence](../06_Coherence_and_Consistency/01_Cache_Coherence.md); the fabric realization lives in [ACE_and_CHI](../06_Coherence_and_Consistency/03_ACE_and_CHI.md).*
...
- **Down the stack (what this is built from):** [Memory](../00_Design_Methodology/02_CPU_PPA_and_Physical_Implementation.md) (the SRAM cell and DRAM array behind every latency here), [CMOS_Fundamentals](../../../00_Fundamentals/01_CMOS_Fundamentals.md) (the FO4 budget and leakage that bound §2's hit path and §9's power techniques), [DDR_Controller](../../04_SoC_and_Chiplet_Architecture/02_Shared_Memory/01_DDR_Controller.md) (the $t_{DRAM}$ and bandwidth a last-level miss actually pays).
- **Up the stack (what builds on this):** [OoO_Execution](../03_Out_of_Order_Backend/01_OoO_Execution.md) (the LSQ and window that generate the MLP §3 exploits and set the MSHR ceiling via the same Little's law), [TLB_and_Virtual_Memory](../05_Virtual_Memory/01_TLB_and_Virtual_Memory.md) (translation in front of the index/tag split; §1.5 derives the VIPT ceiling $C\le N\!\times\!P$, its §6 owns the overlap mechanism and synonyms), [Cache_Coherence](../06_Coherence_and_Consistency/01_Cache_Coherence.md) (the full controller this page motivates), [ACE_and_CHI](../06_Coherence_and_Consistency/03_ACE_and_CHI.md) (the transport and scaling of that controller's messages), [CPU_Architecture §8](../01_Core_Foundations/01_CPU_Architecture.md) (the architectural contract), [CPU workload and performance methods](../00_Design_Methodology/01_CPU_Workloads_Performance_and_DSE.md) (where AMAT, the reuse-distance miss curve, and MLP feed design-space exploration), [Xiangshan_CPU_Design](../07_Core_Case_Studies/01_Xiangshan_CPU_Design.md) (a complete open core composing this hierarchy).
- **Adjacent:** [CPU_Architecture](../01_Core_Foundations/01_CPU_Architecture.md) (the AMAT baseline and pipeline this deepens).
