# Cache Coherence — From Modified, Exclusive, Shared, Invalid (MESI) States to a Correct Controller

> **First-time reader orientation:** When CPU cores cache the same address, coherence controls read and write permission so they do not keep incompatible values. MESI names four stable permissions—Modified, Exclusive, Shared, and Invalid—but real controllers also need transient states for requests in flight. The chapter separates permissions, messages, data movement, races, and proof obligations.
...
> miss status holding register (MSHR); static random-access memory (SRAM); double data rate (DDR); content-addressable memory (CAM); error-correcting code (ECC);
> level-one cache (L1); last-level cache (LLC); direct memory access (DMA); Advanced eXtensible Interface (AXI); Advanced High-performance Bus (AHB);
> Advanced Peripheral Bus (APB); Advanced Microcontroller Bus Architecture (AMBA); AXI Coherency Extensions (ACE); Coherent Hub Interface (CHI); Modified, Owned, Exclusive, Shared, Invalid (MOESI);
> program counter (PC); Shared-to-Modified transient state (SM); read-modify-write (RMW); virtually indexed, physically tagged (VIPT); gigabyte (GB);
> mebibyte (MiB); gigahertz (GHz).

> **Prerequisites:** [CPU_Architecture](../01_Core_Foundations/01_CPU_Architecture.md) §8–§9 (the single-writer/multiple-reader (SWMR) and data-value contracts, plus the distinction between coherence and consistency), [Cache_Microarchitecture](../04_Cache_Hierarchy/01_Cache_Microarchitecture.md) (tags, miss-status holding registers (MSHRs), write-back, inclusion), and [AHB_AXI_APB](../../04_SoC_and_Chiplet_Architecture/03_Transaction_Protocols/01_AHB_AXI_APB.md) (requests, responses, and backpressure).
> **Hands off to:** [ACE_and_CHI](03_ACE_and_CHI.md) (AXI Coherency Extensions (ACE) and Coherent Hub Interface (CHI): how snoop/directory transactions ride a real fabric), [Network_on_Chip](../../04_SoC_and_Chiplet_Architecture/04_On_Chip_Networks/01_Network_on_Chip.md) (routing, virtual channels, and protocol deadlock), and [gem5](../08_Simulation/01_gem5.md) §4 (executable protocol models in Ruby and the Specification Language for Implementing Cache Coherence (SLICC)).
...
This chapter bridges the gap between the protocol-level view in [CPU_Architecture §8](../01_Core_Foundations/01_CPU_Architecture.md) and the fabric-level realization in [ACE_and_CHI](03_ACE_and_CHI.md). It answers the questions a microarchitect or verification engineer faces:
...
This is **protocol deadlock**, distinct from the channel-dependency deadlock of wormhole routing. [Network_on_Chip §4](../../04_SoC_and_Chiplet_Architecture/04_On_Chip_Networks/01_Network_on_Chip.md) supplies the routing theorem; the coherence controller must supply the message-class theorem.
