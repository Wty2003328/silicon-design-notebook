# AXI Coherency Extensions (ACE) and Coherent Hub Interface (CHI) — CPU Cache Coherence at the Interconnect

> **First-time reader orientation:** AXI Coherency Extensions add snoop transactions to an AXI-based system; Coherent Hub Interface uses explicit request, snoop, response, and data channels with request nodes, home nodes, and subordinate nodes. These are CPU-coherence protocols: they transport the ownership and data rules introduced in the preceding coherence chapter across a real scalable fabric.

> **Abbreviation key — skim now and return as needed:** central processing unit (CPU); graphics processing unit (GPU); neural processing unit (NPU); system on chip (SoC); design-space exploration (DSE);
> out-of-order (OoO); miss status holding register (MSHR); load-store queue (LSQ); static random-access memory (SRAM); dynamic random-access memory (DRAM);
> high-bandwidth memory (HBM); double data rate (DDR); level-one cache (L1); level-two cache (L2); last-level cache (LLC);
> network on chip (NoC); virtual network (VN); direct memory access (DMA); Advanced eXtensible Interface (AXI); Advanced High-performance Bus (AHB);
> Advanced Peripheral Bus (APB); Advanced Microcontroller Bus Architecture (AMBA); Modified, Exclusive, Shared, Invalid (MESI); Modified, Owned, Exclusive, Shared, Invalid (MOESI); Compute Express Link (CXL);
> Universal Chiplet Interconnect Express (UCIe); Peripheral Component Interconnect Express (PCIe); die-to-die (D2D); finite-state machine (FSM); artificial intelligence (AI);
> read-modify-write (RMW); non-uniform memory access (NUMA); physical-layer interface (PHY); input/output (I/O); megabyte (MB);
> gigabyte (GB); megahertz (MHz); gigahertz (GHz).
...
3. **Make a multi-message transaction appear atomic.** Because a request, its snoops, the snoop responses, the data, and the completion acknowledgment span many cycles, the protocol must handle the window in which a line is *in transit*. The cache controller records transient permissions and unfinished obligations in a transaction buffer entry (TBE); the fabric's ordering point serializes same-line transactions and delivers the messages that discharge those obligations. [Cache_Coherence §3–§5](01_Cache_Coherence.md) owns the controller side of that handshake.
...
SWMR says the writable set is empty or a singleton that *excludes every reader*; data-value says every read returns the most recent write in the per-line order $\prec_x$ (ordering *across* addresses is [§9](../01_Core_Foundations/01_CPU_Architecture.md)'s separate consistency contract). [CPU_Architecture §8](../01_Core_Foundations/01_CPU_Architecture.md) derives the **state** set from these invariants by a counting argument — MSI is correctness-minimal, $+$E is traffic-minimal, $+$O enables cache-to-cache dirty supply — and we do not repeat it. This page owns the dual object the state derivation implies: the minimal **message** set the fabric must carry to *move* a line between those states without ever transiently breaking SWMR or data-value.
...
The rate $r$ is not free traffic — it is the **coherence miss**, the *fourth* C beyond the compulsory/capacity/conflict misses of a uniprocessor cache ([Cache_Microarchitecture](../04_Cache_Hierarchy/01_Cache_Microarchitecture.md)): a miss caused not by the cache being too small but by *another core's write invalidating a line this core still wanted*. It exists only because sharing exists, and it is the entire reason a coherence fabric carries traffic at all. It has two sources — one irreducible, one pure waste:
...
| **Instruction-cache invalidate** | L1 I-cache, which in Arm is **not required** to be coherent with the D-side | a core writes new instructions through the data side, the instruction side keeps serving the old bytes, and self-modifying code, JITs, debuggers, and module loaders all silently execute stale code | `IC IALLUIS`; RISC-V's counterpart is `FENCE.I` (the Zifencei extension), which is architecturally **local to the executing hart** — the same local-only property §14.0 notes for `SFENCE.VMA`, so it cannot be carried as a fabric operation and every other hart must be made to execute its own, by IPI or by an SBI (Supervisor Binary Interface) remote-fence call |
...
- **Batch the fence instead of broadcasting the invalidate — RISC-V `Svinval`.** §14.0 ended by noting that `SFENCE.VMA` is local, so a RISC-V shootdown still pays an IPI. The ratified **Svinval** extension does not change that; it attacks the *other* term. A plain `SFENCE.VMA` is two things at once — an invalidation **and** an ordering fence that must place all prior page-table stores before it and all subsequent implicit page-walk reads after it — so a loop of $n$ of them serializes the walker and the store path $n$ times. Svinval separates them: `SINVAL.VMA` performs the invalidation with **no** ordering guarantee, `SFENCE.W.INVAL` is issued **once** before the batch to order prior PTE writes ahead of it, and `SFENCE.INVAL.IR` **once** after to order the invalidations ahead of subsequent implicit reads. `HINVAL.VVMA` and `HINVAL.GVMA` are the hypervisor-extension forms for guest stage-1 and stage-2 entries. Unmapping §14.2's 512-page region on one hart then costs $2$ fences plus $512$ unordered invalidations rather than $512$ fenced ones, so the per-operation ordering cost is amortized over $n$ instead of paid $n$ times. What it does **not** buy: `SINVAL.VMA` still reaches only the issuing hart's translation caches, so the IPI to each of the other $N-1$ harts and its 500–2000-cycle handler entry (§14.0) survives intact. Svinval shrinks the constant on each hart's local sweep; only a broadcast, DVM-like transaction attacks the $O(N)$ that §14.5 quantifies.
...
- **Down the stack (what this fabric is built from):** [Network_on_Chip](../../04_SoC_and_Chiplet_Architecture/04_On_Chip_Networks/01_Network_on_Chip.md) (the mesh topology, credit-based flow control §3, routing/protocol deadlock and virtual networks §4, and the coherent-mesh/CMN realization §7 that CHI rides on; D2D physical layer §8), [AHB_AXI_APB](../../04_SoC_and_Chiplet_Architecture/03_Transaction_Protocols/01_AHB_AXI_APB.md) (the AXI4 channels and handshakes ACE extends), [Memory](../00_Design_Methodology/02_CPU_PPA_and_Physical_Implementation.md) & [DDR_Controller](../../04_SoC_and_Chiplet_Architecture/02_Shared_Memory/01_DDR_Controller.md) (the SN-F memory endpoint), [IC_Packaging](../../../07_Manufacturing_and_Bringup/02_IC_Packaging.md) (the chiplet/D2D substrate of §7).
- **Up the stack (what defines the contract this realizes):** [CPU_Architecture](../01_Core_Foundations/01_CPU_Architecture.md) §8 (the SWMR/Data-Value invariants and the MESI/MOESI *state*-count derivation whose *message*-set dual is §1.1 here — this page names and diagrams those same five states as CHI's I/UC/UD/SC/SD in §4.3, and walks their canonical read-miss transaction flow with the `CompAck` ordering handshake in §4.4) and §9 (the consistency model the fabric's ordering honors) — specifically §8.5 for the core-side *latency* of the ping-pong whose *fabric-bandwidth* cost is derived in §3.1, [Cache_Microarchitecture](../04_Cache_Hierarchy/01_Cache_Microarchitecture.md) §8 (the in-cache coherence controller, MSHRs, and inclusion policy on the other side of the snoop; the four-Cs miss taxonomy and 64 B line geometry behind §3.1/§5.1).
- **Controller dual:** [Cache_Coherence](01_Cache_Coherence.md) owns the transient states/TBEs, same-line races, acknowledgement proof, directory encodings, protocol deadlock, and safety/liveness properties that these fabric transactions must satisfy.
- **Adjacent / consumers:** [OoO_Execution](../03_Out_of_Order_Backend/01_OoO_Execution.md) §5 (the LSQ that enforces per-core ordering the fabric completes), [GPU_Architecture](../../02_GPU_Architecture/01_Core_Architecture/01_GPU_Architecture.md) & [NPU_Accelerators](../../03_NPU_Architecture/01_Compute_Dataflows/01_NPU_Accelerators.md) (accelerators that become CXL coherence clients, §7), [Xiangshan_CPU_Design](../07_Core_Case_Studies/01_Xiangshan_CPU_Design.md) (a complete core that plugs into such a fabric), [CPU workload and performance methods](../00_Design_Methodology/01_CPU_Workloads_Performance_and_DSE.md) §4.4 (the Universal Scalability Law contention term the §3.1 coherence miss feeds; where these scaling models feed system design-space exploration).
...
| Direction | Link |
|---|---|
| Prerequisite | [CPU_Architecture](../01_Core_Foundations/01_CPU_Architecture.md) (coherence contract §8–§9) |
| Prerequisite | [Cache_Coherence](01_Cache_Coherence.md) (controller and correctness) |
| Prerequisite | [AHB_AXI_APB](../../04_SoC_and_Chiplet_Architecture/03_Transaction_Protocols/01_AHB_AXI_APB.md) (AXI4 substrate) |
