...
- **CDC:** multi-clock SoCs put async FIFOs at NIU boundaries or run the whole fabric on one mesochronous grid ([Async_Circuit_Design](../03_Frontend_RTL_and_Verification/Async_Circuit_Design.md)).
...
- **Die-to-die:** UCIe/proprietary D2D PHYs carry the same flit protocol across chiplets (CHI C2C, AXI-over-D2D) — the NoC becomes a *package-scale* network; add ~5–20 ns and a serdes-style PHY per crossing ([IC_Packaging](../07_Manufacturing_and_Bringup/IC_Packaging.md)).
- **Floorplan coupling:** router placement = tile placement; bisection wires must physically exist across the die middle — a mesh's $k \cdot b$ bisection is also a *routing-resource* statement ([Physical_Design](../05_Backend_Physical_Design/Physical_Design.md)).
...
- Components: [Memory](Memory.md) (FIFOs, SRAM buffers), [Async_Circuit_Design](../03_Frontend_RTL_and_Verification/Async_Circuit_Design.md) (CDC at NIUs).
- Implementation: [Physical_Design](../05_Backend_Physical_Design/Physical_Design.md) (link pipelining, floorplan), [IC_Packaging](../07_Manufacturing_and_Bringup/IC_Packaging.md) (UCIe die-to-die).
