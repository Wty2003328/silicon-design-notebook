...
Accelerator simulation is stratified not by "how detailed" alone but by *what unit of work* the model reasons about. Three rungs, mapping onto the general fidelity ladder of [Simulation_Methodology §2](../../05_Architecture_Foundations_and_Methods/05_Simulation_Methodology/01_Simulation_Methodology.md):
...
A *dataflow* is the equivalence class of mappings that hold the same tensor stationary in the PEs. The three canonical ones — plus input-stationary, which several tools implement — are summarized as a reuse table in [Perf_Modeling §10.2](../../05_Architecture_Foundations_and_Methods/02_Performance_Analysis/01_Performance_Modeling_and_DSE.md); here is the mechanism behind that table, because it is what the mapping tools are searching over.
...
- **Down the stack:** [Simulation_Methodology](../../05_Architecture_Foundations_and_Methods/05_Simulation_Methodology/01_Simulation_Methodology.md) (functional/timing/physical, trace vs execution, discrete-event contention — the paradigm vocabulary), [DDR_Controller](../../04_SoC_and_Chiplet_Architecture/02_Shared_Memory/01_DDR_Controller.md) (the FR-FCFS/JEDEC DRAM model SCALE-Sim v3 pulls in via Ramulator), [Memory §12b](../../05_Architecture_Foundations_and_Methods/04_Hardware_Structures/01_Memory_Arrays_and_Technologies.md) (compute-in-memory, the accelerator substrate NeuroSim models — see [Other_Architecture_Simulators](../../05_Architecture_Foundations_and_Methods/06_Tool_Landscape/01_Other_Architecture_Simulators.md)).
- **Up the stack:** [Performance_Modeling_and_DSE §9–11](../../05_Architecture_Foundations_and_Methods/02_Performance_Analysis/01_Performance_Modeling_and_DSE.md) (systolic/occupancy/roofline kernels, dataflow table, and the full NeuSim worked example — this page defers rather than duplicates its operator-cost roofline-max [§11.1](../../05_Architecture_Foundations_and_Methods/02_Performance_Analysis/01_Performance_Modeling_and_DSE.md), SA/VU-allocation law [§11.2](../../05_Architecture_Foundations_and_Methods/02_Performance_Analysis/01_Performance_Modeling_and_DSE.md), ring-AllReduce $2(p{-}1)/p$ derivation [§11.3](../../05_Architecture_Foundations_and_Methods/02_Performance_Analysis/01_Performance_Modeling_and_DSE.md), and the "why analytical, not cycle-accurate" trade [§11.4](../../05_Architecture_Foundations_and_Methods/02_Performance_Analysis/01_Performance_Modeling_and_DSE.md)), [Full_Chip_Modeling §4](../../04_SoC_and_Chiplet_Architecture/01_System_Modeling/01_Full_Chip_Modeling.md) (PE→array→core→chip→pod composition with power/thermal; the tool map that places these simulators), [Block_Activity_and_Power](../../../02_Power_and_Low_Power/02_Block_Activity_and_Power.md) (the activity × per-event-energy method Accelergy applies).
- **Sibling pages (this folder):** [Other_Architecture_Simulators](../../05_Architecture_Foundations_and_Methods/06_Tool_Landscape/01_Other_Architecture_Simulators.md) (manycore/CPU, NoC, CIM/neuromorphic, datacenter, chiplet, RISC-V), and the gem5 / DRAM / GPU per-tool pages.
...
- NeuSim (UIUC PlatformX) — [github.com/platformxlab/NeuSim](https://github.com/platformxlab/NeuSim) (full treatment in [Performance_Modeling_and_DSE §11](../../05_Architecture_Foundations_and_Methods/02_Performance_Analysis/01_Performance_Modeling_and_DSE.md)).
