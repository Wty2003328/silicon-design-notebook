...
and the DRAM term dominates the *mapping-dependent* energy: a worse map that spilled partial sums (weight-stationary, $R_C\!\sim\!K/D$, [NPU §3](../01_Compute_Dataflows/01_NPU_Accelerators.md)) lifts $N_{\text{DRAM}}$ to $2MK+2KN+\tfrac{K}{D}MN=3.93\times10^{5}$ and $E_{\text{DRAM}}$ to $7.86\times10^{7}$ — a $20\%$ energy swing from the *same MACs*, which is exactly the quantity Timeloop searches to minimize and Accelergy prices. The whole analytical loop is these two arithmetic steps: **Timeloop computes $N_{c,a}$ from the tiling, Accelergy supplies $e_{c,a}$, the dot product is the energy.**
