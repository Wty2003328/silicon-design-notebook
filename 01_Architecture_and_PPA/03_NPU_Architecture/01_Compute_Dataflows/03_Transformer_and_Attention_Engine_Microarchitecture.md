...
An exponential unit may use range reduction plus a polynomial or table approximation — the full design space (range reduction, minimax polynomials, table-plus-interpolation, and CORDIC) is detailed in [Floating_Point §8.7](../../../00_Fundamentals/04_Floating_Point.md). Accuracy policy is part of architecture: acceptable error, rounding mode, exceptional inputs, and accumulation width must match model-quality validation.
