...
> **Prerequisites:** [Simulation_Methodology](01_Simulation_Methodology.md) (the paradigm/fidelity vocabulary this page instantiates for accelerators), [Performance_Modeling_and_DSE §9–11](../01_Modeling/01_Performance_Modeling_and_DSE.md) (systolic-array cycle model, dataflow taxonomy, the NeuSim worked example this page extends).
> **Hands off to:** [Full_Chip_Modeling §4](../01_Modeling/02_Full_Chip_Modeling.md) (composing a PE→array→chip→pod NPU model with power/thermal), the companion AI-infra notebook for GPU/TPU microarchitecture.
...
where $e_\ell$ = energy per access at level $\ell$ (a register read is ~$10^2\times$ cheaper than a DRAM access) and $e_{\text{MAC}}$ = energy per MAC. Because $e_{\text{DRAM}} \gg e_{\text{SRAM}} \gg e_{\text{RF}}$, **energy is dominated by where the accesses land, which the mapping controls — so the mapping is far more an energy decision than a cycle decision.** This is the accelerator analogue of the activity × per-event-energy model that McPAT applies to CPUs ([Full_Chip_Modeling §1.5](../01_Modeling/02_Full_Chip_Modeling.md)); Accelergy (§4) is literally that model for accelerators.
...
A *dataflow* is the equivalence class of mappings that hold the same tensor stationary in the PEs. The three canonical ones — plus input-stationary, which several tools implement — are summarized as a reuse table in [Perf_Modeling §10.2](../01_Modeling/01_Performance_Modeling_and_DSE.md); here is the mechanism behind that table, because it is what the mapping tools are searching over.
...
- **Cycle-accurate DRAM via Ramulator** — replaces v2's flat bandwidth assumption with a real bank/rank/channel + JEDEC-timing + FR-FCFS model ([DDR_Controller](../03_Memory/04_DDR_Controller.md)), so $\text{stall}_{\text{mem}}$ reflects actual DRAM scheduling, not an average.
...
- **Down the stack:** [Simulation_Methodology](01_Simulation_Methodology.md) (functional/timing/physical, trace vs execution, discrete-event contention — the paradigm vocabulary), [DDR_Controller](../03_Memory/04_DDR_Controller.md) (the FR-FCFS/JEDEC DRAM model SCALE-Sim v3 pulls in via Ramulator), [Memory §12b](../03_Memory/03_Memory.md) (compute-in-memory, the accelerator substrate NeuroSim models — see [Other_Architecture_Simulators](06_Other_Architecture_Simulators.md)).
- **Up the stack:** [Performance_Modeling_and_DSE §9–11](../01_Modeling/01_Performance_Modeling_and_DSE.md) (systolic/occupancy/roofline kernels, dataflow table, and the full NeuSim worked example — this page defers rather than duplicates its operator-cost roofline-max [§11.1](../01_Modeling/01_Performance_Modeling_and_DSE.md), SA/VU-allocation law [§11.2](../01_Modeling/01_Performance_Modeling_and_DSE.md), ring-AllReduce $2(p{-}1)/p$ derivation [§11.3](../01_Modeling/01_Performance_Modeling_and_DSE.md), and the "why analytical, not cycle-accurate" trade [§11.4](../01_Modeling/01_Performance_Modeling_and_DSE.md)), [Full_Chip_Modeling §4](../01_Modeling/02_Full_Chip_Modeling.md) (PE→array→core→chip→pod composition with power/thermal; the tool map that places these simulators), [Block_Activity_and_Power](../../02_Power_and_Low_Power/02_Block_Activity_and_Power.md) (the activity × per-event-energy method Accelergy applies).
...
- NeuSim (UIUC PlatformX) — [github.com/platformxlab/NeuSim](https://github.com/platformxlab/NeuSim) (full treatment in [Performance_Modeling_and_DSE §11](../01_Modeling/01_Performance_Modeling_and_DSE.md)).
