...
> **Prerequisites:** [Simulation_Methodology](01_Simulation_Methodology.md) (why a closed-form is the top rung of the fidelity ladder), [Performance_Modeling_and_DSE](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md) (the CPI stack §2.1, Amdahl §2.2, roofline §2.3/§9.3, occupancy/Little §9.1 — **this page extends those kernels, it does not repeat them**).
> **Hands off to:** [gem5](../02_CPU_and_System/01_gem5.md) and the other per-tool pages — the executable models you escalate to when the closed form stops distinguishing your choices.
...
## 2. Roofline — the throughput bound (extends [DSE §2.3](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md))

The per-kernel GPU treatment and the DSE *application* live on the [DSE §2.3](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md) page; this page carries the **rigorous bound**, its tightness condition, and the cache-aware form.
...
The ceiling is set by the **serial residue** $1-p$ alone: at $p=0.95$ even infinite workers give only $1/0.05=20\times$. (Full sensitivity — $dS_\infty/dp=S_\infty^2$, the last-percent-of-serial lever — is [DSE §2.2](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md).)
...
The **wait alone** $W_q=\frac{\rho}{1-\rho}T_0$ diverges as $\rho\to1$ because draining a backlog needs a run of shorter-than-average inter-arrival gaps, and such runs get exponentially rarer as arrivals approach capacity. ([Full_Chip §2.2](../../01_Modeling/02_System_and_PPA/01_Full_Chip_Modeling.md) runs the same chain at the DRAM channel.)
...
which is set by the **interface cost $(o+L)$**, and is *essentially independent of $A$* for large $A$. The architect's takeaway: **a faster accelerator does not lower the size at which offload pays — the interface does.** A 100× engine behind a high-overhead bus is worthless for small tiles; you must either amortize over large $g$ or cut $(o+L)$. This is the analytical dual of the accelerator/offload cost that shows up in [AHB_AXI_APB](../../04_Interconnect/01_Protocols/01_AHB_AXI_APB.md)/[ACE_and_CHI](../../04_Interconnect/01_Protocols/02_ACE_and_CHI.md) and in the NPU dataflow models of [DSE §10](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md).
...
The failure mode in both directions is real: **trusting an analytical *ceiling* as a *prediction*** (roofline attainment without checking MLP) over-promises; **cycle-simulating a question a closed form already answers** wastes a CPU-week and, worse, hides the mechanism the equation would have made obvious. The mature workflow is **prune analytically, then cycle-simulate the survivors** — the DSE recipe of [DSE §5](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md).
...
- **Down the stack:** [Performance_Modeling_and_DSE](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md) (the working kernels this page makes rigorous — CPI stack, Amdahl, roofline, occupancy), [OoO_Execution](../../02_CPU/03_Out_of_Order_Backend/01_OoO_Execution.md) (the ROB/window structures the interval model abstracts), [Memory](../../03_Memory/04_Memory_Technologies/01_Memory_Arrays_and_Technologies.md) / [DDR_Controller](../../03_Memory/05_Main_Memory/01_DDR_Controller.md) & [Network_on_Chip](../../04_Interconnect/02_Network_on_Chip/01_Network_on_Chip.md) (the queues M/M/1 abstracts; the schedulers that cut $C_v$), [ACE_and_CHI](../../04_Interconnect/01_Protocols/02_ACE_and_CHI.md) (the coherence traffic behind the USL $\kappa$ term).
- **Up the stack (where these models are *applied*):** [Simulation_Methodology](01_Simulation_Methodology.md) (the fidelity ladder these models top, and the escalation rule), [Full_Chip_Modeling](../../01_Modeling/02_System_and_PPA/01_Full_Chip_Modeling.md) (these leaf models composed into a chip — the M/M/1–M/D/1 queue of §5 at the DRAM channel is its §2.2, and the USL peak of §4 is its §4.2), [Performance_Modeling_and_DSE](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md) (roofline *applied* per-kernel §2.3/§9.3 and per-operator §10; Amdahl/Gustafson §2.2).
- **Sibling (this folder):** [gem5](../02_CPU_and_System/01_gem5.md) — the executable form of the interval (O3), queueing (Ruby/memory), and roofline (DSE) models here; [Other_Architecture_Simulators §2](../05_Specialized_Simulators/01_Other_Architecture_Simulators.md) — **Sniper**, the interval model of §3 made the timing engine itself; escalate to either per §7.
