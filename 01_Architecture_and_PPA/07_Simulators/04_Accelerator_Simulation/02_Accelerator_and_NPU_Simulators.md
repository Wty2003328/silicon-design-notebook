...
Accelerator simulation is stratified not by "how detailed" alone but by *what unit of work* the model reasons about. Three rungs, mapping onto the general fidelity ladder of [Simulation_Methodology §2](../01_Methodology/01_Simulation_Methodology.md):
...
where $e_\ell$ = energy per access at level $\ell$ (a register read is ~$10^2\times$ cheaper than a DRAM access) and $e_{\text{MAC}}$ = energy per MAC. Because $e_{\text{DRAM}} \gg e_{\text{SRAM}} \gg e_{\text{RF}}$, **energy is dominated by where the accesses land, which the mapping controls — so the mapping is far more an energy decision than a cycle decision.** This is the accelerator analogue of the activity × per-event-energy model that McPAT applies to CPUs ([Full_Chip_Modeling §1.5](../../01_Modeling/02_System_and_PPA/01_Full_Chip_Modeling.md)); Accelergy (§4) is literally that model for accelerators.
...
A *dataflow* is the equivalence class of mappings that hold the same tensor stationary in the PEs. The three canonical ones — plus input-stationary, which several tools implement — are summarized as a reuse table in [Perf_Modeling §10.2](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md); here is the mechanism behind that table, because it is what the mapping tools are searching over.
...
**Why this forces sampling — a population-independent coverage bound.** At $\sim\!10^{5}$–$10^{6}$ mappings/s an exhaustive sweep of $5.3\times10^{8}$ is minutes but $10^{15}$ is millennia, so Timeloop instead draws *random valid mappings* and keeps the best. The guarantee is the order-statistic twin of the CLT sampling bound of [Simulation_Methodology §5.1](../01_Methodology/01_Simulation_Methodology.md): $n$ uniform draws *all* miss the top-$q$ fraction of mappings with probability $(1-q)^{n}$, so landing in the top-$q$ with confidence $1-\delta$ needs

$$n \;\ge\; \frac{\ln\delta}{\ln(1-q)} \;\approx\; \frac1q\ln\frac1\delta\quad(\text{small }q),$$

where $q$ = target quantile and $\delta$ = miss probability — **independent of $|\mathcal M|$.** Hitting the top $0.1\%$ ($q{=}10^{-3}$) with $99\%$ confidence ($\delta{=}10^{-2}$) needs $n\ge\ln(0.01)/\ln(0.999)\approx4.6\times10^{3}$ mappings — the *same* few thousand whether the space is $10^{8}$ or $10^{15}$. That population-independence is exactly why heuristic sampling, not exhaustive search, is the mapper's engine, and it is the accelerator instance of the §5 DSE funnel ([Perf_Modeling §5](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md)): prune analytically, then sample the survivors under a computable confidence.
...
- **Cycle-accurate DRAM via Ramulator** — replaces v2's flat bandwidth assumption with a real bank/rank/channel + JEDEC-timing + FR-FCFS model ([DDR_Controller](../../03_Memory/05_Main_Memory/01_DDR_Controller.md)), so $\text{stall}_{\text{mem}}$ reflects actual DRAM scheduling, not an average.
...
- **Down the stack:** [Simulation_Methodology](../01_Methodology/01_Simulation_Methodology.md) (functional/timing/physical, trace vs execution, discrete-event contention — the paradigm vocabulary), [DDR_Controller](../../03_Memory/05_Main_Memory/01_DDR_Controller.md) (the FR-FCFS/JEDEC DRAM model SCALE-Sim v3 pulls in via Ramulator), [Memory §12b](../../03_Memory/04_Memory_Technologies/01_Memory_Arrays_and_Technologies.md) (compute-in-memory, the accelerator substrate NeuroSim models — see [Other_Architecture_Simulators](../05_Specialized_Simulators/01_Other_Architecture_Simulators.md)).
- **Up the stack:** [Performance_Modeling_and_DSE §9–11](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md) (systolic/occupancy/roofline kernels, dataflow table, and the full NeuSim worked example — this page defers rather than duplicates its operator-cost roofline-max [§11.1](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md), SA/VU-allocation law [§11.2](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md), ring-AllReduce $2(p{-}1)/p$ derivation [§11.3](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md), and the "why analytical, not cycle-accurate" trade [§11.4](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md)), [Full_Chip_Modeling §4](../../01_Modeling/02_System_and_PPA/01_Full_Chip_Modeling.md) (PE→array→core→chip→pod composition with power/thermal; the tool map that places these simulators), [Block_Activity_and_Power](../../../02_Power_and_Low_Power/02_Block_Activity_and_Power.md) (the activity × per-event-energy method Accelergy applies).
- **Sibling pages (this folder):** [Other_Architecture_Simulators](../05_Specialized_Simulators/01_Other_Architecture_Simulators.md) (manycore/CPU, NoC, CIM/neuromorphic, datacenter, chiplet, RISC-V), and the gem5 / DRAM / GPU per-tool pages.
...
- NeuSim (UIUC PlatformX) — [github.com/platformxlab/NeuSim](https://github.com/platformxlab/NeuSim) (full treatment in [Performance_Modeling_and_DSE §11](../../01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md)).
