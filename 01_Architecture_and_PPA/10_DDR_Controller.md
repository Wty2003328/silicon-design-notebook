...
> **Prerequisites:** [Memory](09_Memory.md) (the 1T1C cell, charge sharing, sense amplifier, refresh *physics* this page treats as given), [CMOS_Fundamentals](../00_Fundamentals/01_CMOS_Fundamentals.md) (signal integrity, the power-grid droop that timing constraints protect), [Cache_Microarchitecture](07_Cache_Microarchitecture.md) (the row buffer read as a one-entry cache).
> **Hands off to:** [DRAM_Simulators](06_Simulators/03_DRAM_Simulators.md) (the same timing, policies, and queueing turned into an executable model), [AHB_AXI_APB](11_AHB_AXI_APB.md) (the AXI/CHI bus the requestors arrive on), [CPU_Architecture](03_CPU_Architecture.md) (the memory stalls this latency becomes).
...
Everything else in a datasheet is a variation on these events, and the full analog collapse of them into constants is exactly what a [DRAM simulator abstracts](06_Simulators/03_DRAM_Simulators.md).
...
- **QoS** reweights age for real-time requestors (display, camera) that have hard deadlines a throughput-optimal schedule would miss; the [AXI](11_AHB_AXI_APB.md) `AxQOS` tag carries the priority in.
...
- **Down the stack (what this controller is built on):** [Memory](09_Memory.md) — the 1T1C cell, charge sharing, sense amplifier, and refresh physics that §2–§3 treat as timing constants; [CMOS_Fundamentals](../00_Fundamentals/01_CMOS_Fundamentals.md) — the signal integrity and power-grid droop that $t_{RCD}$, $t_{RP}$, and $t_{FAW}$ protect (§3).
- **Up the stack (what builds on it):** [DRAM_Simulators](06_Simulators/03_DRAM_Simulators.md) — the same bank FSM, JEDEC guards, FR-FCFS, and queueing model as an *executable* cycle-level tool (its §8 formalizes §7.3 here); [Cache_Microarchitecture](07_Cache_Microarchitecture.md) — the row buffer as a one-entry cache and the 64 B line that sets burst granularity (§9.1); [OoO_Execution](05_OoO_Execution.md) — the ROB/MLP window sized against the *loaded* DRAM latency of §7.3; [AHB_AXI_APB](11_AHB_AXI_APB.md) — the AXI/CHI interface and `AxQOS` the requestors and §5.2 QoS ride on; [CPU_Architecture](03_CPU_Architecture.md) — where this latency becomes a pipeline stall.
- **Adjacent:** [Performance_Modeling_and_DSE](01_Performance_Modeling_and_DSE.md) — where these bandwidth/latency models feed design-space exploration; [GPU_Architecture](15_GPU_Architecture.md) & [NPU_Accelerators](16_NPU_Accelerators.md) — the GDDR/HBM and LPDDR5X variants of the same controller under different bandwidth/energy objectives (§9.2).
