...
> **Prerequisites:** [CPU_Architecture](03_CPU_Architecture.md) (pipeline, memory hierarchy, the AMAT baseline of its §7), [Memory](09_Memory.md) (SRAM cell, DRAM organization), [OoO_Execution](05_OoO_Execution.md) (the instruction window that generates the memory-level parallelism §3 exploits).
> **Hands off to:** [ACE_and_CHI](12_ACE_and_CHI.md) (coherence protocols at scale), [DDR_Controller](10_DDR_Controller.md) (the DRAM a last-level miss talks to), [TLB_and_Virtual_Memory](08_TLB_and_Virtual_Memory.md) (the translation in front of every access).

---

## 0. Why this page exists

A cache exists to fight one number: the gap between a core cycle (~0.2–0.3 ns) and a DRAM access (~60–100 ns). That gap — the **memory wall** — is 100–300 core cycles wide, long enough to drain any realistic instruction window and leave the machine idle. The out-of-order core ([OoO_Execution](05_OoO_Execution.md)) hides *some* of that latency behind a large window; the cache attacks the latency *itself*, and manages to do so cheaply only because real programs are not random.
...
where $\lambda_{miss}$ = misses issued per cycle, $L_{miss}$ = miss latency. But $k$ is not free to grow: the *independent* misses available to overlap are limited by how far the instruction window can run ahead, which the ROB caps ([OoO_Execution §3.2](05_OoO_Execution.md)). So MSHR count and window size are co-designed — an L1D that could track 40 misses is pointless behind a window that only ever exposes 8. This is why real MSHR counts track the level's realistic MLP, not the theoretical maximum:
...
*Scope: the idea and where it lands in a cache. Full MESI/MOESI transition tables, snoop walkthroughs, and directory scaling live in [CPU_Architecture §8](03_CPU_Architecture.md) and [ACE_and_CHI](12_ACE_and_CHI.md).*
...
- **Down the stack (what this is built from):** [Memory](09_Memory.md) (the SRAM cell and DRAM array behind every latency here), [CMOS_Fundamentals](../00_Fundamentals/01_CMOS_Fundamentals.md) (the FO4 budget and leakage that bound §2's hit path and §9's power techniques), [DDR_Controller](10_DDR_Controller.md) (the $t_{DRAM}$ and bandwidth a last-level miss actually pays).
- **Up the stack (what builds on this):** [OoO_Execution](05_OoO_Execution.md) (the LSQ and window that generate the MLP §3 exploits and set the MSHR ceiling), [TLB_and_Virtual_Memory](08_TLB_and_Virtual_Memory.md) (translation in front of the index/tag split), [ACE_and_CHI](12_ACE_and_CHI.md) & [CPU_Architecture §8](03_CPU_Architecture.md) (the full coherence protocols §8 only sketches), [Performance_Modeling_and_DSE](01_Performance_Modeling_and_DSE.md) (where AMAT and MLP feed design-space exploration), [Xiangshan_CPU_Design](14_Xiangshan_CPU_Design.md) (a complete open core composing this hierarchy).
- **Adjacent:** [CPU_Architecture](03_CPU_Architecture.md) (the AMAT baseline and pipeline this deepens).
