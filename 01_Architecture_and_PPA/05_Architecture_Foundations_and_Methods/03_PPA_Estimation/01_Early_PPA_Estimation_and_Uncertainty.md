# Early Power, Performance, and Area (PPA) Estimation and Uncertainty

> **First-time reader orientation:** Before register-transfer level hardware or a gate-level netlist exists, architects estimate power, performance, and area using structural proxies and calibrated reference blocks. Early values should be ranges with explicit assumptions. Sensitivity analysis identifies which unknown can reverse the decision; later implementation results narrow and recalibrate those ranges.

> **Abbreviation key — skim now and return as needed:** register-transfer level (RTL); design-space exploration (DSE); translation lookaside buffer (TLB); reorder buffer (ROB); miss status holding register (MSHR);
> issue queue (IQ); arithmetic logic unit (ALU); static random-access memory (SRAM); dynamic random-access memory (DRAM); content-addressable memory (CAM);
> first in, first out (FIFO); error-correcting code (ECC); network on chip (NoC); virtual channel (VC); processing element (PE);
> multiply-accumulate (MAC); input/output (I/O); gigahertz (GHz).

> **Prerequisites:** [Full-Chip Modeling](../../04_SoC_and_Chiplet_Architecture/01_System_Modeling/01_Full_Chip_Modeling.md) (hierarchical composition and coupling), [Performance Modeling and DSE](../02_Performance_Analysis/01_Performance_Modeling_and_DSE.md) (objective functions), and [Power Fundamentals](../../../02_Power_and_Low_Power/01_Power_Fundamentals.md).
...
## Before the details: early estimates are structured arguments

Before layout exists, area cannot be measured directly and power depends on unknown capacitance, activity, voltage, frequency, and temperature. Architects therefore decompose the design into recognizable structures—memory arrays, queues, arithmetic units, control logic, interconnect, clocking, and physical overhead—and scale from relevant reference implementations or analytical models.

The estimate should distinguish **model uncertainty** from **design variation**. Model uncertainty asks how wrong the proxy or calibration may be. Design variation asks how the result changes across plausible workloads, configurations, process, voltage, or frequency. A point estimate hides both; an interval plus sensitivity table shows whether a choice remains good across them. Correlation matters because frequency, voltage, power, and temperature move together.

**Beginner checkpoint:** write every estimate as value, range, assumptions, calibration source, and update milestone. More decimal places do not add information. The useful question is whether uncertainty can reverse the architecture decision and which next measurement will reduce that risk most.
...
- **Inputs:** [Workload Characterization and Sampling](../02_Performance_Analysis/02_Workload_Characterization_and_Sampling.md), [Performance Modeling and DSE](../02_Performance_Analysis/01_Performance_Modeling_and_DSE.md).
- **Composition:** [Full-Chip Modeling](../../04_SoC_and_Chiplet_Architecture/01_System_Modeling/01_Full_Chip_Modeling.md), [Block Activity and Power](../../../02_Power_and_Low_Power/02_Block_Activity_and_Power.md).
