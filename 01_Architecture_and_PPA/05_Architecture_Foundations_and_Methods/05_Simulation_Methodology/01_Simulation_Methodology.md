# Simulation Methodology — How Architecture Simulators Work

> **First-time reader orientation:** A simulator is an executable model, not an automatically trustworthy replica. Functional models compute correct results; timing models estimate when events occur; trace-driven models replay recorded behavior; execution-driven models let timing alter later behavior. Region selection, warm-up, calibration, validation, statistics, and an explicit error budget determine whether results support a decision.
...
> register-transfer level (RTL); instructions per cycle (IPC); cycles per instruction (CPI); memory-level parallelism (MLP); design-space exploration (DSE);
> out-of-order (OoO); translation lookaside buffer (TLB); reorder buffer (ROB); miss status holding register (MSHR); load-store queue (LSQ);
> issue queue (IQ); arithmetic logic unit (ALU); dynamic random-access memory (DRAM); double data rate (DDR); level-two cache (L2);
> last-level cache (LLC); network on chip (NoC); least recently used (LRU); first come, first served (FCFS); dynamic voltage and frequency scaling (DVFS);
> deep neural network (DNN); general-purpose computing on graphics processing units (GPGPU); reliability, availability, and serviceability (RAS); floating point (FP); million instructions per second (MIPS);
> initiation interval (II); region of interest (ROI); megabyte (MB); gigabyte (GB); gigahertz (GHz).

> **Prerequisites:** [Performance_Modeling_and_DSE](../02_Performance_Analysis/01_Performance_Modeling_and_DSE.md) (the analytical leaf models a simulator makes executable), [Full_Chip_Modeling](../../04_SoC_and_Chiplet_Architecture/01_System_Modeling/01_Full_Chip_Modeling.md) (the tool/fidelity ladder this page explains conceptually).
...
3. **Physical — "what does it cost?"** Overlay energy, power, area, temperature onto the timed activity. This is the McPAT / Accelergy / DRAMPower / HotSpot layer covered in [Full_Chip_Modeling](../../04_SoC_and_Chiplet_Architecture/01_System_Modeling/01_Full_Chip_Modeling.md).
...
- **Cycle-level DRAM model** (Ramulator, DRAMSim3): models banks/ranks/channels, the Joint Electron Device Engineering Council (JEDEC) timing constraints ($t_{RCD}, t_{RP}, t_{RAS}, t_{FAW}, t_{WTR}, \dots$), the row-buffer, refresh, and the **scheduler** (first-ready, first-come, first-served (FR-FCFS), which reorders to hit open rows). Achieved bandwidth is an *output* of this contention, not an input — a memory-bound number that came from a fixed-latency model is not credible.

The theoretical backbone across all of it is **queueing**: a shared resource of service rate $\mu$ under offered load $\lambda$ has utilization $\rho=\lambda/\mu$, and mean latency grows as $\sim 1/(1-\rho)$ — latency runs to the knee as you approach saturation. Every credible memory/NoC simulator is, at bottom, a structural realization of that curve; a model that reports latency flat with load has no queue and is optimistic. (DRAM device/controller detail: [Memory](../04_Hardware_Structures/01_Memory_Arrays_and_Technologies.md), [DDR_Controller](../../04_SoC_and_Chiplet_Architecture/02_Shared_Memory/01_DDR_Controller.md).)
...
- **Calibration** fits free parameters (latencies, penalties, McPAT's technology knobs) to close the gap; an *un-calibrated* architectural model can be 2× off, a calibrated one ~20–30% (see the [Full_Chip_Modeling §1.7 accuracy ladder](../../04_SoC_and_Chiplet_Architecture/01_System_Modeling/01_Full_Chip_Modeling.md)).
