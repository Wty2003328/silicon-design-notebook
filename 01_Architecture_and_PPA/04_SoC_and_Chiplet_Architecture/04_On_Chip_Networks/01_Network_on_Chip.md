...
> **First-time reader orientation:** A network on chip (NoC) divides messages into packets and smaller flow-control units called flits. Routers move flits across links according to a topology and routing rule. It replaces a long shared bus when many endpoints need concurrent communication, but introduces queueing, routing, backpressure, and deadlock concerns.

> **Abbreviation key — skim now and return as needed:** central processing unit (CPU); graphics processing unit (GPU); system on chip (SoC); instructions per cycle (IPC); design-space exploration (DSE);
> out-of-order (OoO); reorder buffer (ROB); static random-access memory (SRAM); dynamic random-access memory (DRAM); double data rate (DDR);
> first in, first out (FIFO); level-two cache (L2); last-level cache (LLC); network on chip (NoC); virtual channel (VC);
> virtual network (VN); quality of service (QoS); direct memory access (DMA); Advanced eXtensible Interface (AXI); Advanced High-performance Bus (AHB);
> Advanced Peripheral Bus (APB); AXI Coherency Extensions (ACE); Coherent Hub Interface (CHI); Universal Chiplet Interconnect Express (UCIe); die-to-die (D2D);
> virtual address (VA); streaming multiprocessor (SM); complementary metal-oxide-semiconductor (CMOS); clock-domain crossing (CDC); artificial intelligence (AI);
> physical-layer interface (PHY); input/output (I/O); gigabyte (GB); gigahertz (GHz).

> **Prerequisites:** [AHB_AXI_APB](../03_Transaction_Protocols/01_AHB_AXI_APB.md) (the bus and crossbar the NoC replaces), [ACE_and_CHI](../../01_CPU_Architecture/06_Coherence_and_Consistency/03_ACE_and_CHI.md) (the coherence protocol that rides on it), [Memory](../00_Design_Methodology/02_SoC_Chiplet_PPA_and_Physical_Implementation.md) (the SRAM/FIFO buffers a router is built from).
> **Hands off to:** [GPU_Architecture](../../02_GPU_Architecture/01_Core_Architecture/01_GPU_Architecture.md) (the SM↔L2 fabric), [Physical_Design](../../../05_Backend_Physical_Design/01_Physical_Design.md) (link pipelining and floorplan coupling), [IC_Packaging](../../../07_Manufacturing_and_Bringup/02_IC_Packaging.md) (UCIe die-to-die extension).
...
where $D_{\min}$ = buffers per VC for full throughput and $t_{crt}$ = credit round-trip (its three terms are the flit-forward, credit-return, and counter-update latencies). Under-buffer and the link throttles with *zero* contention — sustained utilization is $D/t_{crt}$, so a 3-cycle loop with 2 buffers caps a link at 67 % for no reason but under-provisioning (the classic "why is my link at 60 %" bug). This is the same **bandwidth-delay product** that sizes AXI outstanding transactions ([AHB_AXI_APB](../03_Transaction_Protocols/01_AHB_AXI_APB.md)) and the ROB ([OoO_Execution](../../01_CPU_Architecture/03_Out_of_Order_Backend/01_OoO_Execution.md)): buffer $\ge$ rate $\times$ round-trip, or the pipe runs dry.
...
**Buffer sizing is the router.** Input buffers dominate router area and power: $v$ VCs $\times$ $d$ flits $\times$ port, at 256–512-bit flits, gives a 5-port router **~40–160 Kb** of SRAM/flops ([Memory](../00_Design_Methodology/02_SoC_Chiplet_PPA_and_Physical_Implementation.md)). NoC power is **~5–15 %** of SoC dynamic power in agent-heavy designs, and *over half of it* is buffers and links — so the whole buffer-shrinking chain (wormhole → few flits/VC → credit-sized depth, §3) is not a detail, it is the reason the fabric fits in budget.
...
- **Down the stack (what the fabric is built from):** [Memory](../00_Design_Methodology/02_SoC_Chiplet_PPA_and_Physical_Implementation.md) (the FIFO/SRAM buffers that dominate router area, §5), [CMOS_Fundamentals](../../../00_Fundamentals/01_CMOS_Fundamentals.md) (the wire RC delay that makes the bus slow and the crossbar untimeable, §1), [Async_Design_and_CDC](../../../03_Frontend_RTL_and_Verification/06_Async_Design_and_CDC.md) (async FIFOs at NIU clock crossings, §8), [Physical_Design](../../../05_Backend_Physical_Design/01_Physical_Design.md) (link pipelining, floorplan coupling, the bisection-as-routing-resource claim of §2/§8), [IC_Packaging](../../../07_Manufacturing_and_Bringup/02_IC_Packaging.md) (UCIe die-to-die, §8).
- **Up the stack (what rides on it):** [ACE_and_CHI](../../01_CPU_Architecture/06_Coherence_and_Consistency/03_ACE_and_CHI.md) (the coherence protocol and its REQ/RSP/SNP/DAT VNs, §4/§7), [AHB_AXI_APB](../03_Transaction_Protocols/01_AHB_AXI_APB.md) (the bus/crossbar the NoC replaces and its five-channel outstanding model, §1/§3), [GPU_Architecture](../../02_GPU_Architecture/01_Core_Architecture/01_GPU_Architecture.md) (the SM↔L2 mesh/crossbar and slice hashing, §7), [SoC/chiplet workload and performance methods](../00_Design_Methodology/01_SoC_Chiplet_Workloads_Performance_and_DSE.md) (where the latency-under-load and bisection models feed design-space exploration, §6).
- **Adjacent / system-scale:** [DDR_Controller](../02_Shared_Memory/01_DDR_Controller.md) (channel hashing, the same load-spreading trick as HN-F interleave), datacenter interconnect (same deadlock/queueing theory, bigger wires), and Tenstorrent/Cerebras NoC-as-fabric (companion AI-infra material).
