# Double Data Rate (DDR) Memory Controller — Why Dynamic RAM Needs a Scheduler

> **First-time reader orientation:** Dynamic random-access memory (DRAM) stores bits as charge that must be sensed and refreshed. Double data rate interfaces transfer data on both clock edges. A memory controller translates read/write requests into legal activate, read, write, precharge, and refresh commands while scheduling banks to balance latency, locality, fairness, and bandwidth.
...
> memory-level parallelism (MLP); design-space exploration (DSE); out-of-order (OoO); reorder buffer (ROB); static random-access memory (SRAM);
> dynamic random-access memory (DRAM); high-bandwidth memory (HBM); error-correcting code (ECC); single-error correction, double-error detection (SECDED); quality of service (QoS);
> direct memory access (DMA); Advanced eXtensible Interface (AXI); Advanced High-performance Bus (AHB); Advanced Peripheral Bus (APB); Coherent Hub Interface (CHI);
> first come, first served (FCFS); dynamic voltage and frequency scaling (DVFS); Compute Express Link (CXL); Peripheral Component Interconnect Express (PCIe); reliability, availability, and serviceability (RAS);
> complementary metal-oxide-semiconductor (CMOS); finite-state machine (FSM); artificial intelligence (AI); exclusive OR (XOR); non-uniform memory access (NUMA);
> physical-layer interface (PHY); kilobyte (KB); gigabyte (GB).

> **Prerequisites:** [Memory](../00_Design_Methodology/02_SoC_Chiplet_PPA_and_Physical_Implementation.md) (the 1T1C cell, charge sharing, sense amplifier, refresh *physics* this page treats as given), [CMOS_Fundamentals](../../../00_Fundamentals/01_CMOS_Fundamentals.md) (signal integrity, the power-grid droop that timing constraints protect), [Cache_Microarchitecture](../../01_CPU_Architecture/04_Cache_Hierarchy/01_Cache_Microarchitecture.md) (the row buffer read as a one-entry cache).
> **Hands off to:** [DRAM_Simulators](../06_Simulation/01_DRAM_Simulators.md) (the same timing, policies, and queueing turned into an executable model), [AHB_AXI_APB](../03_Transaction_Protocols/01_AHB_AXI_APB.md) (the AXI/CHI bus the requestors arrive on), [CPU_Architecture](../../01_CPU_Architecture/01_Core_Foundations/01_CPU_Architecture.md) (the memory stalls this latency becomes).

---

## 0. Why this page exists

**"Random Access Memory" is a lie for DRAM.** The load/store abstraction the core issues — a flat array with uniform latency and no history — describes an SRAM. A DRAM *device* is a stateful, timing-constrained, self-erasing, shared analog machine, and it violates every clause of the flat-RAM contract: its access latency depends on what it did last, its commands are illegal unless spaced by device physics, reading it destroys the data, its cells forget on a millisecond clock, and one channel is fought over by every core, the GPU, and the DMA (direct memory access) engines at once.

The memory controller is the entire apparatus that manufactures the flat-RAM illusion on top of that device — and the *imperfection* of the illusion **is** the memory-system performance model. This page derives the controller from the four ways DRAM breaks the abstraction (§1), then builds up the structures each break forces: the bank as a state machine whose row-hit/miss/conflict split sets the whole cost model (§2), the Joint Electron Device Engineering Council (JEDEC) timing constraints as the *physics each one protects* rather than a parameter table (§3), open-vs-closed page as a locality *prediction* (§4), first-ready, first-come, first-served (FR-FCFS) scheduling as the resolution of a locality-vs-fairness trade (§5), refresh as a mandatory background tax (§6), and the achieved-bandwidth-and-loaded-latency model that ties it together with the same queueing intuition the ROB (reorder buffer) uses (§7). ECC/RAS (reliability, availability, serviceability) (§8) and DDR5/low-power DDR generation 5X (LPDDR5X) (§9) close it out as concepts. What is deliberately *cut*: the command-encoding truth tables, the per-signal pin lists, the controller-FSM RTL, and the address-bit dumps — none of them teach *why* the controller is shaped the way it is. The executable form of everything here lives on the [DRAM_Simulators](../06_Simulation/01_DRAM_Simulators.md) page; this page is the hardware and the physics.
...
- **Bus turnaround ($t_{WTR}$, $t_{RTW}$) protects the shared bidirectional DQ (data bus) bus.** DQ is source-synchronous and driven from both ends; reversing direction means stopping one driver, re-settling termination/ODT (on-die termination), re-aligning the DLL (delay-locked loop) capture phase, and guarding against two drivers overlapping (a dead short). The several-ns cost of a read↔write switch is the entire reason writes are *batched* (§5.2).
...
Everything else in a datasheet is a variation on these events, and the full analog collapse of them into constants is exactly what a [DRAM simulator abstracts](../06_Simulation/01_DRAM_Simulators.md).
...
- **QoS** reweights age for real-time requestors (display, camera) that have hard deadlines a throughput-optimal schedule would miss; the [AXI](../03_Transaction_Protocols/01_AHB_AXI_APB.md) (Advanced eXtensible Interface) `AxQOS` tag carries the priority in.
...
This is **Little's law for the memory system** — the same bandwidth-delay-product argument that sizes an OoO ROB ([OoO_Execution §3.2](../../01_CPU_Architecture/03_Out_of_Order_Backend/01_OoO_Execution.md)), with $t_{RC}$ playing the role of miss latency and banks playing the role of MLP (memory-level parallelism). It explains the bank counts directly: DDR4 provides 16 banks per rank and DDR5 provides 32 (two subchannels × 16) precisely so that random, miss-heavy traffic has enough independent banks to approach peak.
...
- **Down the stack (what this controller is built on):** [Memory](../00_Design_Methodology/02_SoC_Chiplet_PPA_and_Physical_Implementation.md) — the 1T1C cell, charge sharing, sense amplifier, and refresh physics that §2–§3 treat as timing constants; [CMOS_Fundamentals](../../../00_Fundamentals/01_CMOS_Fundamentals.md) — the signal integrity and power-grid droop that $t_{RCD}$, $t_{RP}$, and $t_{FAW}$ protect (§3).
- **Up the stack (what builds on it):** [DRAM_Simulators](../06_Simulation/01_DRAM_Simulators.md) — the same bank FSM, JEDEC guards, FR-FCFS, and queueing model as an *executable* cycle-level tool (its §8 formalizes §7.3 here); [Cache_Microarchitecture](../../01_CPU_Architecture/04_Cache_Hierarchy/01_Cache_Microarchitecture.md) — the row buffer as a one-entry cache and the 64 B line that sets burst granularity (§9.1); [OoO_Execution](../../01_CPU_Architecture/03_Out_of_Order_Backend/01_OoO_Execution.md) — the ROB/MLP window sized against the *loaded* DRAM latency of §7.3; [AHB_AXI_APB](../03_Transaction_Protocols/01_AHB_AXI_APB.md) — the AXI/CHI interface and `AxQOS` the requestors and §5.2 QoS ride on; [CPU_Architecture](../../01_CPU_Architecture/01_Core_Foundations/01_CPU_Architecture.md) — where this latency becomes a pipeline stall.
- **Adjacent:** [SoC/chiplet workload and performance methods](../00_Design_Methodology/01_SoC_Chiplet_Workloads_Performance_and_DSE.md) uses these delivered-bandwidth, latency, bank-parallelism, and queueing mechanisms in system design-space exploration; [GPU Architecture](../../02_GPU_Architecture/01_Core_Architecture/01_GPU_Architecture.md) and [NPU Accelerators](../../03_NPU_Architecture/01_Compute_Dataflows/01_NPU_Accelerators.md) apply GDDR/HBM and LPDDR variants under different bandwidth/energy objectives.
