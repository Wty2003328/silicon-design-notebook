...
**Q (T-Head):** RISC-V's privilege modes — and why is machine mode said to sit *below* the OS?
**A:** Three modes: M (machine, `11`), S (supervisor, `01`), U (user, `00`). M is the most privileged, but the useful mental picture is that it sits *beneath* the kernel rather than above it, because firmware, a security monitor, a trusted execution environment, and the supervisor binary interface (SBI) all need isolation **from** the operating system, not merely from applications — otherwise a kernel compromise undoes secure boot and attestation too. The stack is modular: a deeply embedded part implements M only and omits S, page tables, and sometimes even PMP. The only upward edges are traps; the only downward edges are `mret`/`sret`. → [Privileged_Architecture_CSRs_and_Traps](../01_Architecture_and_PPA/01_CPU_Architecture/01_Core_Foundations/04_Privileged_Architecture_CSRs_and_Traps.md) §2.

**Q (Cambricon):** Walk an Sv39 translation and give the supported page sizes.
**A:** `satp` holds MODE (0 = Bare, 8 = Sv39, 9 = Sv48, 10 = Sv57), an ASID, and the root table's physical page number. Sv39 slices the 39-bit virtual address into three 9-bit indices plus a 12-bit offset, so a walk is **three dependent memory reads** — each PTE holds the next table's address, so they cannot be overlapped, only removed. A leaf at the last level maps **4 KB**; a leaf found one level early is a **2 MB** superpage and ends the walk in two reads; two levels early is a **1 GB** superpage in one read. Sv48 and Sv57 each add one level, buying 512× the address space for one more dependent access. → [TLB_and_Virtual_Memory](../01_Architecture_and_PPA/01_CPU_Architecture/05_Virtual_Memory/01_TLB_and_Virtual_Memory.md) §5, §7; `satp` fields in [Privileged_Architecture_CSRs_and_Traps](../01_Architecture_and_PPA/01_CPU_Architecture/01_Core_Foundations/04_Privileged_Architecture_CSRs_and_Traps.md) §5.

**Q (Zhaoxin):** Does `SFENCE.VMA` invalidate the other cores' TLBs?
**A:** No — it is architecturally **local to the executing hart**, with operands only to scope it by virtual page number and/or ASID. TLBs are not kept coherent by hardware, so a multicore shootdown is a software protocol: the modifying hart publishes the page-table change, sends inter-processor interrupts (in RISC-V typically through an SBI remote-fence service), every target executes its own `SFENCE.VMA` and acknowledges, and the initiator waits. The cost is `t_IPI + (N−1)·t_flush + t_sync = O(N)` with every core stalled, so wasted work is `O(N²)`; mitigations are ASID tagging, directed shootdown to only the cores that hold the mapping, and batching. `FENCE.I` is local in exactly the same way. → [TLB_and_Virtual_Memory](../01_Architecture_and_PPA/01_CPU_Architecture/05_Virtual_Memory/01_TLB_and_Virtual_Memory.md) §8.

**Q (T-Head):** LR/SC versus AMO, and what do the `.aq` / `.rl` bits do?
**A:** An AMO (`amoadd`, `amoswap`, `amomax`, …) is one unconditional read-modify-write performed at a serialization point — the core's L1 with the line locked, or the home node near memory. LR/SC is an optimistic load that arms a reservation plus a store-conditional that fails if the reservation was disturbed: the cheapest possible hardware (one address register and a comparator) and universal like compare-and-swap, but an unbounded retry loop, which is why Zacas later added `amocas`. The A extension is formally Zaamo (AMOs) + Zalrsc (LR/SC), independently implementable. Ordering is *separate* from atomicity and rides in two instruction bits: neither set = relaxed, `.aq` = acquire, `.rl` = release, `.aqrl` = sequentially consistent. They are independently settable, so a release-only unlock skips the acquire machinery — a distinction x86, where every atomic is both, cannot express. → [Atomic_Operations](../01_Architecture_and_PPA/01_CPU_Architecture/06_Coherence_and_Consistency/04_Atomic_Operations.md) §3.5, §5, §7.

**Q (Huawei):** How does a RISC-V hart find out *which* device interrupted?
**A:** The core itself exposes only a few wires — machine/supervisor external, timer, software. The CLINT (core-local interruptor) supplies the timer interrupt (`mtime` compared against a per-hart `mtimecmp`) and the software interrupt (`msip`, writable by any hart, so it is also the IPI mechanism). Everything else arrives as the single external interrupt, and the PLIC (platform-level interrupt controller) names the source: up to 1023 wired sources, per-**context** enables and threshold where a context is a (hart, privilege level) pair, and a claim/complete register. **Claim is a read with a side effect** — it atomically returns the highest-priority pending enabled ID *and* clears that pending bit — because a passive read plus a separate clear would let two harts enabled for the same source both run the handler. Writing the ID back completes it and lets the gateway forward the next request. The PLIC has no active state and no hardware preemption, so software nesting costs four extra MMIO accesses; the AIA (advanced interrupt architecture) replaces it for large systems with APLIC (wired sources, optionally converted to MSIs) plus a per-hart IMSIC interrupt file. → [Interrupt_Architecture](../01_Architecture_and_PPA/04_SoC_and_Chiplet_Architecture/05_IO_and_Chiplets/05_Interrupt_Architecture.md) §6.

**Q (Cambricon):** What is PMP, and how is it different from an MMU?
**A:** Physical memory protection is a small set of address-range checks applied to the **physical** address, in parallel with the L1 tag compare — no page tables, no translation, no address space. It is what protects M-mode's memory from S and U on parts that have no MMU at all, and what protects it before the MMU is configured. Each entry's `pmpcfg.A` selects TOR (top of range: the previous entry's address is the base), NA4 (a single 4-byte region), or NAPOT (naturally aligned power of two, with the size encoded as a run of low-order ones in `pmpaddr`). **The lowest-numbered matching entry wins**, so a narrow read-only carve-out at index 0 overrides a permissive region at index 1 — and reordering entries silently changes policy, which is a documented software obligation rather than a hardware guarantee. It is not free: a 64-entry, 56-bit-physical-address block is roughly 40–50 kGE, larger than a small RV32IMC core. → [Privileged_Architecture_CSRs_and_Traps](../01_Architecture_and_PPA/01_CPU_Architecture/01_Core_Foundations/04_Privileged_Architecture_CSRs_and_Traps.md) §10.
