...
| **RISC-V privilege modes** | **M / S / U** (machine / supervisor / user), encoded `11` / `01` / `00`. M is the most privileged but sits *below* the OS — firmware, a security monitor, and the SBI (supervisor binary interface) live there, so a kernel compromise is not a total compromise. An embedded part may implement M only. |
| **RISC-V Sv39 paging** | `satp` = MODE (8 = Sv39) + ASID + root page-table PPN. A 39-bit VA splits into 3 × 9-bit indices + 12-bit offset → **3 dependent memory reads**. Leaf at the last level = **4 KB**; one level early = **2 MB** (2 reads); two early = **1 GB** (1 read). Sv48/Sv57 add a level each. |
| **`SFENCE.VMA`** | Architecturally **local to the executing hart**. A multicore TLB shootdown is therefore software: send inter-processor interrupts (IPIs), each target executes its own `SFENCE.VMA` and acknowledges. Cost `t_IPI + (N−1)·t_flush + t_sync = O(N)`. `FENCE.I` is local in the same way. |
| **LR/SC vs AMO** | AMO (`amoadd`, `amoswap`, …) = one unconditional read-modify-write at a serialization point. LR/SC = optimistic reserve + conditional store; cheapest hardware (one address register + comparator) and universal, but an *unbounded* retry loop — which is why Zacas later added `amocas`. The A extension = **Zaamo** (AMOs) + **Zalrsc** (LR/SC). |
| **RISC-V `.aq` / `.rl`** | Two independently settable ordering bits on an atomic, separate from its atomicity: neither = relaxed, `.aq` = acquire, `.rl` = release, `.aqrl` = sequentially consistent. Independence is the point — a release-only unlock skips acquire machinery; x86, where every atomic is both, cannot express it. |
| **PLIC / CLINT / AIA** | CLINT = per-hart timer (`mtime` vs `mtimecmp`) + software interrupt (`msip`), nothing else. PLIC names *which* device fired: ≤1023 wired sources, per-**context** (hart × privilege) enables and threshold, and a **claim** register whose *read* atomically returns the top pending ID and clears it (atomic, or two harts run one handler); writing the ID back **completes** it. No active state, no hardware preemption. AIA = **APLIC** (wired, can emit MSIs) + **IMSIC** (per-hart message-signaled file). |
| **C (compressed) extension** | 16-bit encodings of the commonest instructions → **25–30 %** static code shrink, not 50 %. Mean size `4 − 2·f_c` bytes with `f_c ≈ 0.5–0.6` of static instructions compressible; the tail cannot, because compressed forms name only `x8`–`x15` and carry narrow immediates. Real motive is often front-end bandwidth and I-cache reach (~1.4×). |
| **Extensions & profiles** | Frozen base RV32I/RV64I + optional letters — M mul/div, A atomics, F/D float, C compressed, V vector, B bitmanip, H hypervisor. **RV64G** = I + M + A + F + D + Zicsr + Zifencei. **Profiles** re-freeze a bundle so software has one target: **RVA23** mandates V, bitmanip, crypto and more for application-class chips. |
| **PMP** | Physical memory protection: address-range checks on the **physical** address, in parallel with the L1 tag compare, no page tables. Modes **TOR** (previous entry is the base), **NA4** (4 bytes), **NAPOT** (naturally aligned power of two, size = run of low `pmpaddr` ones). **Lowest matching index wins**, so reordering entries silently changes policy. |

Deep dives: [Cache Microarchitecture](../01_Architecture_and_PPA/01_CPU_Architecture/04_Cache_Hierarchy/01_Cache_Microarchitecture.md) · [Cache Coherence](../01_Architecture_and_PPA/01_CPU_Architecture/06_Coherence_and_Consistency/01_Cache_Coherence.md) · [TLB & Virtual Memory](../01_Architecture_and_PPA/01_CPU_Architecture/05_Virtual_Memory/01_TLB_and_Virtual_Memory.md) · [RISC-V ISA](../01_Architecture_and_PPA/01_CPU_Architecture/01_Core_Foundations/02_RISC_V_ISA.md) · [Privileged Architecture, CSRs & Traps](../01_Architecture_and_PPA/01_CPU_Architecture/01_Core_Foundations/04_Privileged_Architecture_CSRs_and_Traps.md) · [Atomic Operations](../01_Architecture_and_PPA/01_CPU_Architecture/06_Coherence_and_Consistency/04_Atomic_Operations.md) · [Interrupt Architecture](../01_Architecture_and_PPA/04_SoC_and_Chiplet_Architecture/05_IO_and_Chiplets/05_Interrupt_Architecture.md) · [01 Arch Questions](01_Architecture_and_PPA_Questions.md).
...
| **TileLink** | The RISC-V ecosystem's open on-chip protocol; tiers by *conformance level* instead of separate specs — **TL-UL** (uncached lightweight: single-word, no bursts), **TL-UH** (+bursts, atomics, hints), **TL-C** (+cache coherence). Five channels **A/B/C/D/E**; TL-UL/UH use only A and D, TL-C uses all five because **B and C carry coherence permissions** — where AMBA bolts coherence on separately as ACE. Rocket Chip, BOOM, XiangShan; OpenTitan uses TL-UL. |
...
→ [On-Chip Transaction Protocols](../01_Architecture_and_PPA/04_SoC_and_Chiplet_Architecture/03_Transaction_Protocols/01_AHB_AXI_APB.md) · [01 Arch Questions](01_Architecture_and_PPA_Questions.md) · [10 §9–10](10_Digital_IC_Frontend_Interview_Bank.md).
