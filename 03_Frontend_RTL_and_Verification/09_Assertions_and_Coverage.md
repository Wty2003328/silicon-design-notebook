...
(The `disable iff(!rst_n)` clause exempts reset — a standard reset gate; the AXI valid/ready contract these encode lives in [AHB_AXI_APB](../01_Architecture_and_PPA/04_Interconnect/01_Protocols/01_AHB_AXI_APB.md).)
...
- **Down the stack (what these mechanisms observe and are built from):** [RTL_Design_Methodology](01_RTL_Design_Methodology.md) (the DUT whose invariants become assertions), [Data_Types_and_Basics](02_Data_Types_and_Basics.md) (the 4-state `x` that `$isunknown` checks), [Procedural_Processes_and_IPC](03_Procedural_Processes_and_IPC.md) (the event scheduler and regions that define *sampled* semantics), [AHB_AXI_APB](../01_Architecture_and_PPA/04_Interconnect/01_Protocols/01_AHB_AXI_APB.md) (the valid/ready protocol contract the interface assertions of §3 encode).
