...
Modern digital design is **synchronous**: state changes only on a clock edge, and the only timing you must prove is setup/hold at flops ([STA](../06_Signoff/01_STA.md)). The rules that make this hold:
...
5. **Separate sequential from combinational** intent: `always_ff` for state, `always_comb` for logic. The SystemVerilog `_ff`/`_comb`/`_latch` variants make the tool *check* your intent ([Procedural_Processes_and_IPC](03_Procedural_Processes_and_IPC.md)).
...
- **Pipeline to hit frequency.** If a combinational path is too long for the target clock ([Performance_Modeling_and_DSE](../01_Architecture_and_PPA/01_Performance_Modeling_and_DSE.md)), cut it with a pipeline register and manage the added latency with `valid`/back-pressure.
...
RTL is not "done" when it simulates — it is done when it is **lint-clean** ([Lint_CDC_RDC_Signoff](07_Lint_CDC_RDC_Signoff.md)), **CDC-clean**, **coverage-closed** ([Verification_Planning_and_Coverage_Closure](11_Verification_Planning_and_Coverage_Closure.md)), and **synthesizes within the timing/area budget**. That is the contract handed to [synthesis](../04_Synthesis/01_Synthesis_and_Optimization.md).
...
## Cross-references
- Language mechanics: [Data_Types_and_Basics](02_Data_Types_and_Basics.md), [Procedural_Processes_and_IPC](03_Procedural_Processes_and_IPC.md), [OOP_and_Randomization](08_OOP_and_Randomization.md).
- Next gates: [Lint_CDC_RDC_Signoff](07_Lint_CDC_RDC_Signoff.md), [Verification_Planning_and_Coverage_Closure](11_Verification_Planning_and_Coverage_Closure.md), [Async_Design_and_CDC](06_Async_Design_and_CDC.md).
- Downstream: [Synthesis_and_Optimization](../04_Synthesis/01_Synthesis_and_Optimization.md), low-power [UPF_Power_Intent](../02_Power_and_Low_Power/04_UPF_Power_Intent.md).
- Drills: [RTL_Coding_Questions](../interview_prep/08_RTL_Coding_Questions.md).
