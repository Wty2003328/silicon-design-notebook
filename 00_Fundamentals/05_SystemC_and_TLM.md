...
> **Stage:** 00 · Fundamentals — the modeling *substrate* the architecture pages use. This is the language/kernel underneath the **virtual platforms** and **transaction-level models** that [Performance_Modeling_and_DSE §4](../01_Architecture_and_PPA/01_Modeling/01_Performance_Modeling_and_DSE.md) commissions and [Simulation_Methodology](../01_Architecture_and_PPA/07_Simulators/01_Simulation_Methodology.md) formalizes.
> **Prerequisites:** [Procedural_Processes_and_IPC](../03_Frontend_RTL_and_Verification/03_Procedural_Processes_and_IPC.md) (the SystemVerilog event scheduler / delta-cycle / NBA-region model — SystemC's evaluate–update is the same idea in a C++ library), [Simulation_Methodology §3](../01_Architecture_and_PPA/07_Simulators/01_Simulation_Methodology.md) (the discrete-event engine and its time-ordered invariant), C++ (templates, virtual dispatch, RAII).
> **Hands off to:** [Performance_Modeling_and_DSE §4](../01_Architecture_and_PPA/01_Modeling/01_Performance_Modeling_and_DSE.md) (LT/AT virtual platforms in the fidelity ladder), [gem5](../01_Architecture_and_PPA/07_Simulators/02_gem5.md) (the SystemC-TLM co-simulation bridge), [AHB_AXI_APB](../01_Architecture_and_PPA/04_Interconnect/01_AHB_AXI_APB.md) & [DDR_Controller](../01_Architecture_and_PPA/03_Memory/04_DDR_Controller.md) (the memory-mapped protocols the generic payload abstracts).

---

## 0. Why this page exists

You cannot boot Linux on a chip that does not exist, and you cannot afford to boot it on the chip's RTL (register-transfer level): a full-SoC RTL simulation runs at ~10²–10⁴ effective Hz ([Simulation_Methodology §2.1](../01_Architecture_and_PPA/07_Simulators/01_Simulation_Methodology.md)), so a 30-second Linux boot — a few tens of billions of target cycles — would take **weeks to months** of wall-clock. Yet firmware, drivers, the BSP (board-support package), and the OS bring-up must be *working* the day silicon returns, or the schedule slips by that same year. The only way out is to model the machine at a coarser rung: throw away the per-cycle, per-net detail and model only the thing software can observe — **memory-mapped transactions** (a CPU reads/writes an address; a device responds). That buys the **100–1000×** over RTL the [Performance_Modeling §4](../01_Architecture_and_PPA/01_Modeling/01_Performance_Modeling_and_DSE.md) ladder quotes for transaction-level modeling (TLM), and with direct-memory and temporal-decoupling tricks (§6) far more — enough to boot an OS in seconds and run software regressions overnight.
...
The separation is the same "build the graph once, then run it" discipline as the static elaboration of an RTL netlist — and for the same reason: a static structure has a fixed cost per event, so the per-event work $i_{\text{ev}}$ of the [slowdown identity $S\approx w=i_{\text{ev}}\,n_{\text{ev}}$](../01_Architecture_and_PPA/07_Simulators/01_Simulation_Methodology.md) is bounded and small.
...
**Why the cost matters — the work-per-event lever.** A native `int32` add is *one* host instruction. An `sc_int<W>` add is that plus a mask-and-sign-extend to $W$ bits (a few instructions); an `sc_bigint<256>` add loops over four 64-bit words; an `sc_lv<W>` add walks per-bit 4-state logic tables. In the [slowdown identity $S\approx i_{\text{ev}}\,n_{\text{ev}}$](../01_Architecture_and_PPA/07_Simulators/01_Simulation_Methodology.md), these types inflate $i_{\text{ev}}$ — the host instructions per modeled operation — directly. A datapath modeled entirely in `sc_lv` can run **10–100× slower** than the same model in native types with a few sized types at the boundaries. The senior rule: **model in native C++ (`uint32_t`, `uint64_t`) by default, and reach for `sc_` types only on the specific signals whose exact width, 4-state, or fixed-point behavior the study actually needs** — the same "pay for fidelity only where a decision depends on it" discipline as the whole [fidelity ladder](../01_Architecture_and_PPA/01_Modeling/01_Performance_Modeling_and_DSE.md).
...
TLM-2.0 names two coding styles, and the choice is the same accuracy↔speed decision as the whole [fidelity ladder](../01_Architecture_and_PPA/01_Modeling/01_Performance_Modeling_and_DSE.md), specialized to bus modeling:
...
Assembling §6.1–§6.2 with the [Simulation_Methodology ladder](../01_Architecture_and_PPA/07_Simulators/01_Simulation_Methodology.md) (all figures illustrative, for a ~1 GHz target):
...
The ladder is **RTL ≪ cycle-accurate ≪ AT < LT < LT+TD+DMI**, spanning ~6 orders of magnitude — the same span as the master fidelity curve, because it *is* that curve applied to system modeling. The "100–1000× over RTL" rule of thumb of [Performance_Modeling §4](../01_Architecture_and_PPA/01_Modeling/01_Performance_Modeling_and_DSE.md) is the AT-to-LT band; DMI + temporal decoupling extend LT past it, which is what makes pre-silicon OS boot and overnight software regression physically possible.

---

## 7. Where it fits — choosing the rung

Every rung answers a different question; the discipline (as everywhere on the [fidelity ladder](../01_Architecture_and_PPA/01_Modeling/01_Performance_Modeling_and_DSE.md)) is **use the fastest model whose fidelity still exceeds the decision you are making.**
...
- **AT — architecture study of the uncore.** Bus arbitration, QoS, interconnect and memory-controller latency-vs-load, buffer/queue sizing — anything set by **contention**, which LT discards. Use AT when the *decision* is about the shared fabric ([AHB_AXI_APB](../01_Architecture_and_PPA/04_Interconnect/01_AHB_AXI_APB.md), [ACE_and_CHI](../01_Architecture_and_PPA/04_Interconnect/02_ACE_and_CHI.md), [Network_on_Chip](../01_Architecture_and_PPA/04_Interconnect/03_Network_on_Chip.md), [DDR_Controller](../01_Architecture_and_PPA/03_Memory/04_DDR_Controller.md)).
- **SystemC-TLM vs gem5.** For CPU-**microarchitecture** performance (IPC, ROB/LSQ pressure), gem5's detailed O3 model is the right tool ([gem5](../01_Architecture_and_PPA/07_Simulators/02_gem5.md), [Simulation_Methodology §6](../01_Architecture_and_PPA/07_Simulators/01_Simulation_Methodology.md)) — SystemC-TLM abstracts the pipeline away. But gem5 ships a **SystemC/TLM bridge**, so you can co-simulate gem5's validated CPU/memory models with existing SystemC-TLM IP (a vendor's peripheral or memory model) — reusing IP that has no gem5 equivalent, at the cost of inheriting gem5's slower detailed models for the parts it owns.
...
- **Up the stack (what commissions this):** [Performance_Modeling_and_DSE §4](../01_Architecture_and_PPA/01_Modeling/01_Performance_Modeling_and_DSE.md) — LT/AT virtual platforms on the coverage/time-to-software axis of the fidelity ladder; this page is the substrate that makes those rungs real. [Full_Chip_Modeling](../01_Architecture_and_PPA/01_Modeling/02_Full_Chip_Modeling.md) — composing leaf models into a chip.
- **Sideways (the engine and its formalization):** [Simulation_Methodology](../01_Architecture_and_PPA/07_Simulators/01_Simulation_Methodology.md) — the discrete-event engine SystemC *is* (§3 event queue + time-ordered invariant → §1; the delta sub-order it predicts → §1.2–§1.3; the slowdown identity $S\approx i_{\text{ev}}n_{\text{ev}}$ → §4, §6.2; contention-as-output → §5.5). [gem5](../01_Architecture_and_PPA/07_Simulators/02_gem5.md) — the CPU-µarch rung and the SystemC-TLM co-sim bridge (§7).
...
- **What the payload abstracts:** [AHB_AXI_APB](../01_Architecture_and_PPA/04_Interconnect/01_AHB_AXI_APB.md) · [ACE_and_CHI](../01_Architecture_and_PPA/04_Interconnect/02_ACE_and_CHI.md) · [Network_on_Chip](../01_Architecture_and_PPA/04_Interconnect/03_Network_on_Chip.md) · [DDR_Controller](../01_Architecture_and_PPA/03_Memory/04_DDR_Controller.md) · [Memory](../01_Architecture_and_PPA/03_Memory/03_Memory.md) — the memory-mapped protocols and devices AT models time and LT models functionally.
