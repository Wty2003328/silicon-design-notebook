...
The separation is the same "build the graph once, then run it" discipline as the static elaboration of an RTL netlist — and for the same reason: a static structure has a fixed cost per event, so the per-event work $i_{\text{ev}}$ of the [slowdown identity $S\approx w=i_{\text{ev}}\,n_{\text{ev}}$](../01_Architecture_and_PPA/07_Simulators/01_Methodology/01_Simulation_Methodology.md) is bounded and small.
...
TLM-2.0 names two coding styles, and the choice is the same accuracy↔speed decision as the whole [fidelity ladder](../01_Architecture_and_PPA/01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md), specialized to bus modeling:
...
Assembling §6.1–§6.2 with the [Simulation_Methodology ladder](../01_Architecture_and_PPA/07_Simulators/01_Methodology/01_Simulation_Methodology.md) (all figures illustrative, for a ~1 GHz target):
...
Every rung answers a different question; the discipline (as everywhere on the [fidelity ladder](../01_Architecture_and_PPA/01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md)) is **use the fastest model whose fidelity still exceeds the decision you are making.**
...
- **Up the stack (what commissions this):** [Performance_Modeling_and_DSE §4](../01_Architecture_and_PPA/01_Modeling/01_Performance_Analysis/01_Performance_Modeling_and_DSE.md) — LT/AT virtual platforms on the coverage/time-to-software axis of the fidelity ladder; this page is the substrate that makes those rungs real. [Full_Chip_Modeling](../01_Architecture_and_PPA/01_Modeling/02_System_and_PPA/01_Full_Chip_Modeling.md) — composing leaf models into a chip.
- **Sideways (the engine and its formalization):** [Simulation_Methodology](../01_Architecture_and_PPA/07_Simulators/01_Methodology/01_Simulation_Methodology.md) — the discrete-event engine SystemC *is* (§3 event queue + time-ordered invariant → §1; the delta sub-order it predicts → §1.2–§1.3; the slowdown identity $S\approx i_{\text{ev}}n_{\text{ev}}$ → §4, §6.2; contention-as-output → §5.5). [gem5](../01_Architecture_and_PPA/07_Simulators/02_CPU_and_System/01_gem5.md) — the CPU-µarch rung and the SystemC-TLM co-sim bridge (§7).
...
- **What the payload abstracts:** [AHB_AXI_APB](../01_Architecture_and_PPA/04_Interconnect/01_Protocols/01_AHB_AXI_APB.md) · [ACE_and_CHI](../01_Architecture_and_PPA/04_Interconnect/01_Protocols/02_ACE_and_CHI.md) · [Network_on_Chip](../01_Architecture_and_PPA/04_Interconnect/02_Network_on_Chip/01_Network_on_Chip.md) · [DDR_Controller](../01_Architecture_and_PPA/03_Memory/05_Main_Memory/01_DDR_Controller.md) · [Memory](../01_Architecture_and_PPA/03_Memory/04_Memory_Technologies/01_Memory_Arrays_and_Technologies.md) — the memory-mapped protocols and devices AT models time and LT models functionally.
